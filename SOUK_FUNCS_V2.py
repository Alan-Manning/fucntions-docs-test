...
Import this package into your python file like any other:

>>> import Maskpy

This contains a main class called souk SOUK_Functions. You should instantiate
this as a new object called something sensible, for example:

>>> souk = Maskpy.SOUK_Functions()

This is now allows the use of all the methods within the class.

This can be instantiated with a different main_cell_name and custom layer
numbering however will just take on default values when left blank.

The default layers used are:
...
>>> Aluminium= 1
    ...
    Backside_Flipped = 29
    Nb_Groundplane = 30
    Pin_hole_positives = 31
    TrimLayer = 90
    Top_choke_waveguide_hole = 150
    Top_choke_anulus = 151
    Bottom_choke_waveguide_hole = 155
    Bottom_choke_IDC_hole = 156
    Bottom_choke_pads = 157
    Tab_dicing_line = 160
    Chip_holder = 175
    General_labeling = 65535

To have custom layer numbering the init should be passed a dictionary with keys
equal the the layer names above and should have a layer and data type dict for
each. For example:

>>> layers_dict = { "Aluminium" : {"layer": 1,   "datatype": 0},
...
class SOUK_Functions:
    ...
    def __init__(self, main_cell_name="MAIN_CELL", layers="Default"):
        ...
        if layers=="Default":
            self.Aluminium                      = {"layer": 1,   "datatype": 0}
            self.Nb_Antenna                     = {"layer": 2,   "datatype": 0}
            self.SiN_dep                        = {"layer": 3,   "datatype": 0}
            self.IDC_Nb                         = {"layer": 4,   "datatype": 0}
            self.SiO                            = {"layer": 5,   "datatype": 0}
            self.SiN_Membrane                   = {"layer": 6,   "datatype": 0}
            self.Backside_Check                 = {"layer": 9,   "datatype": 0}
            self.Backside_Flipped               = {"layer": 29,  "datatype": 0}
            self.Nb_Groundplane                 = {"layer": 30,  "datatype": 0}
            self.Pin_hole_positives             = {"layer": 31,  "datatype": 0}
            self.TrimLayer                      = {"layer": 90,  "datatype": 0}
            self.Top_choke_waveguide_hole       = {"layer": 150, "datatype": 0}
            self.Top_choke_anulus               = {"layer": 151, "datatype": 0}
            self.Bottom_choke_waveguide_hole    = {"layer": 155, "datatype": 0}
            self.Bottom_choke_IDC_hole          = {"layer": 156, "datatype": 0}
            self.Bottom_choke_pads              = {"layer": 157, "datatype": 0}
            self.Tab_dicing_line                = {"layer": 160, "datatype": 0}
            self.Chip_holder                    = {"layer": 175, "datatype": 0}
            self.General_labeling               = {"layer": 65535, "datatype": 0}
        
        elif type(layers)==dict:
            
            self.Aluminium                      = layers["Aluminium"]
            self.Nb_Antenna                     = layers["Nb_Antenna"]
            self.SiN_dep                        = layers["SiN_dep"]
            self.IDC_Nb                         = layers["IDC_Nb"]
            self.SiO                            = layers["SiO"]
            self.SiN_Membrane                   = layers["SiN_Membrane"]
            self.Backside_Check                 = layers["Backside_Check"]
            self.Backside_Flipped               = layers["Backside_Flipped"]
            self.Nb_Groundplane                 = layers["Nb_Groundplane"]
            self.Pin_hole_positives             = layers["Pin_hole_positives"]
            self.TrimLayer                      = layers["TrimLayer"]
            self.Top_choke_waveguide_hole       = layers["Top_choke_waveguide_hole"]
            self.Top_choke_anulus               = layers["Top_choke_anulus"]
            self.Bottom_choke_waveguide_hole    = layers["Bottom_choke_waveguide_hole"]
            self.Bottom_choke_IDC_hole          = layers["Bottom_choke_IDC_hole"]
            self.Bottom_choke_pads              = layers["Bottom_choke_pads"]
            self.Tab_dicing_line                = layers["Tab_dicing_line"]
            self.Chip_holder                    = layers["Chip_holder"]
            self.General_labeling               = layers["General_labeling"]
        else:
            
            raise Exception('layers not in correct format to read. should be dictionary with keys of layer names and values of dictionarys in the form "{"layer": 157, "datatype": 0}"')
    ...
    def generate_hex_pack_circlepack(self, hexagon_radius, hexagon_center, x_pitch, y_pitch):
        ...
        while inside_vert:
            ...
            if gy <= hexagon_center[1]:
                init_x = -hexagon_radius - ((gy - (x_pitch/2) - hexagon_center[0])/np.sqrt(3)) + ((x_pitch/2)/tan(pi/3)) + hexagon_center[0]
    ...
    def generate_half_split_hex_pack(self, hexagon_radius, hexagon_center, x_pitch, y_pitch, middle_gap_distance=0, adjust_center_hex_grid=False, return_two_grids=False, return_split_point=False):
        """
        ...
        for row_number in range(number_of_full_rows):
            ...
            while inside_horiz:
                ...
                if self.inside_hexagon([gx + (x_pitch/2), gy - (y_pitch/2)], hexagon_radius, hexagon_center) and self.inside_hexagon([gx + (x_pitch/2), gy + (y_pitch/2)], hexagon_radius, hexagon_center):
                    if row_number <= number_of_rows_bot:
    ...
    def generate_octagonal_holder_port_dict(self, center_xy, octagon_rotation):
        """
        ...
        oct_ports_dict["0"] = list(self.rotate(center_wafer_x, center_wafer_y, (center_wafer_x - np.sqrt(three_inch**2 - 10000**2)), (center_wafer_y + 10000), self.deg_to_rad(octagon_rotation))) + [octagon_rotation] # sets the x,y and roation of the first two octagon port conection points
        oct_ports_dict["1"] = list(self.rotate(center_wafer_x, center_wafer_y, (center_wafer_x - np.sqrt(three_inch**2 - 10000**2)), (center_wafer_y - 10000), self.deg_to_rad(octagon_rotation))) + [octagon_rotation]                         
    
        for i in range(1,8): # makes the rest of the ports around the octagonal holder
            oct_ports_dict["{0}".format(2*i)] = list(self.rotate(center_wafer_x, center_wafer_y, oct_ports_dict["{0}".format(2*(i-1))][0], oct_ports_dict["{0}".format(2*(i-1))][1], pi/4)) + [(oct_ports_dict["{0}".format(2*(i-1))][2] + 45)%360]
            oct_ports_dict["{0}".format((2*i)+1)] = list(self.rotate(center_wafer_x, center_wafer_y, oct_ports_dict["{0}".format(2*i - 1)][0], oct_ports_dict["{0}".format(2*i - 1)][1], pi/4)) + [(oct_ports_dict["{0}".format(2*i - 1)][2] + 45)%360]
        
        for i in range(0,8): # adds a rectangle around the outside of the wafer shoing where the ports would be+
            self.Main.add(gdspy.Rectangle((oct_ports_dict["{0}".format(2*i)][0]-1000, oct_ports_dict["{0}".format(2*i)][1]-250), (oct_ports_dict["{0}".format(2*i)][0], oct_ports_dict["{0}".format(2*i)][1]+250)).rotate(self.deg_to_rad(oct_ports_dict["{0}".format(2*i)][2]), (oct_ports_dict["{0}".format(2*i)][0], oct_ports_dict["{0}".format(2*i)][1])))
            self.Main.add(gdspy.Rectangle((oct_ports_dict["{0}".format((2*i)+1)][0]-1000, oct_ports_dict["{0}".format((2*i)+1)][1]-250), (oct_ports_dict["{0}".format((2*i)+1)][0], oct_ports_dict["{0}".format((2*i)+1)][1]+250)).rotate(self.deg_to_rad(oct_ports_dict["{0}".format((2*i)+1)][2]), (oct_ports_dict["{0}".format((2*i)+1)][0], oct_ports_dict["{0}".format((2*i)+1)][1])))
    ...
    def make_Toms_6_inch_holder_and_get_ports(self, chip_center_xy, wafer_flat_length, middle_flat_angle=(-pi/2)):
        ...
        for i in range(6):
            ...
            holder_outer_hex_points.append([chip_center_xy[0] + holder_outer_hex_radius*cos(ang_cent_to_edge-(pi/6)),
            ...
            top_edge_port_xy_rot = [chip_center_xy[0] + 1.0025*wafer_radius*cos(ang_cent_to_edge + offset_from_cent_angle),
                                    chip_center_xy[1] + 1.0025*wafer_radius*sin(ang_cent_to_edge + offset_from_cent_angle),
                                    ang_edge_to_cent
                                    ]
            
            mid_edge_port_xy_rot = [chip_center_xy[0] + 1.0025*wafer_radius*cos(ang_cent_to_edge),
                                    chip_center_xy[1] + 1.0025*wafer_radius*sin(ang_cent_to_edge),
                                    ang_edge_to_cent
                                    ]
            
            bot_edge_port_xy_rot = [chip_center_xy[0] + 1.0025*wafer_radius*cos(ang_cent_to_edge - offset_from_cent_angle),
                                    chip_center_xy[1] + 1.0025*wafer_radius*sin(ang_cent_to_edge - offset_from_cent_angle),
                                    ang_edge_to_cent
                                    ]
            ...
            top_port_rect = gdspy.Rectangle([top_edge_port_xy_rot[0] - wafer_radius, top_edge_port_xy_rot[1] + (holder_outer_port_width/2)],
            ...
            mid_port_rect = gdspy.Rectangle([mid_edge_port_xy_rot[0] - wafer_radius, mid_edge_port_xy_rot[1] + (holder_outer_port_width/2)],
            ...
            bot_port_rect = gdspy.Rectangle([bot_edge_port_xy_rot[0] - wafer_radius, bot_edge_port_xy_rot[1] + (holder_outer_port_width/2)],
        ...
        wafer_circle_cutout = self.make_silicon_wafer(wafer_diameter, wafer_flat_length, self.Chip_holder, chip_center=chip_center_xy, flat_angle=middle_flat_angle)
    ...
    def get_SOUK_boundary_hexagon(self):
        ...
        for i in range(6):
            ...
            right_side_point = [hex_point_x + hex_corner_to_straight_length*cos(angle_from_center - (2*pi/3)),
                                hex_point_y + hex_corner_to_straight_length*sin(angle_from_center - (2*pi/3))
                                ]
            
            left_side_point = [hex_point_x + hex_corner_to_straight_length*cos(angle_from_center + (2*pi/3)),
                               hex_point_y + hex_corner_to_straight_length*sin(angle_from_center + (2*pi/3))
                               ]
    ...
    def make_silicon_wafer(self, wafer_diameter, flat_length, layer, chip_center=[0,0], flat_angle=(-pi/2)):
        ...
        flat_cutout_rect = gdspy.Rectangle([middle_of_flat[0] - (flat_length/2), middle_of_flat[1]],
    ...
    def make_test_chip_quadrent_boundary_and_get_horn_positions(self, quadrent_center_xy, Main_config_file_dict, bottom_left_text="", top_right_text="", cardiff_logo=True, top_right_label_window=True):
        """
        ...
        top_right_chip_corner = [quadrent_center_xy[0] + (test_chip_quad_width/2),
                                 quadrent_center_xy[1] + (test_chip_quad_height/2)]
        
        bottom_left_chip_corner = [quadrent_center_xy[0] - (test_chip_quad_width/2),
                                   quadrent_center_xy[1] - (test_chip_quad_height/2)]
        
        bottom_right_chip_corner = [quadrent_center_xy[0] + (test_chip_quad_width/2),
                                    quadrent_center_xy[1] - (test_chip_quad_height/2)]
    ...
    # Adding the bottom_left_text if True
        if bottom_left_text != "":
            ...
            BL_text = gdspy.Text(bottom_left_text , bottom_left_text_size,
                                 position=[bottom_left_chip_corner[0] + bottom_left_text_offset_x,
                                           bottom_left_chip_corner[1] + bottom_left_text_offset_y],
                                 **self.Aluminium)
    ...
    # Adding the top_right_text if True
        if top_right_text != "":
            ...
            TR_text = gdspy.Text(top_right_text , top_right_text_size,
                                 position=[top_right_chip_corner[0] - top_right_text_offset_x,
    ...
    # Adding the top_right_label_window if True
        if top_right_label_window:
            top_right_label_window_rect = gdspy.Rectangle([top_right_chip_corner[0] - top_right_label_window_offset_x, top_right_chip_corner[1] - top_right_label_window_offset_y],
                                                          [top_right_chip_corner[0] - top_right_label_window_offset_x - top_right_label_window_width, top_right_chip_corner[1] - top_right_label_window_offset_y - top_right_label_window_height]
                                                          )
    ...
    # Adding the cardiff logo if True
        if cardiff_logo:
            ...
            logo_xy = [bottom_right_chip_corner[0] - cardiff_logo_offset_x - (cardiff_logo_size/2),
    ...
    # Adding the dicing line in the groundplane and the tabbed dicing line
        ...
        self.add_test_chip_quad_groundplane_edge_cuts(quadrent_center_xy, test_chip_quad_width, test_chip_quad_height,
    ...
    # Adding the center pin and slotted pin
        ...
        slotted_pin_xy = [quadrent_center_xy[0] + slotted_pin_offset_x_from_center_pin,
                          quadrent_center_xy[1] + slotted_pin_offset_y_from_center_pin]
        
        center_pin_bounding_points = self.add_center_pin_and_get_bounding_points(quadrent_center_xy, center_pin_radius)
        
        slotted_pin_bounding_points = self.add_slotted_pin_and_get_bounding_points(slotted_pin_xy, slotted_pin_length, slotted_pin_radius, center_pin_xy)
        
        
        
        
        horn_centers = [[quadrent_center_xy[0] - horn_offset_from_center_x, quadrent_center_xy[1] + horn_offset_from_center_y],
    ...
    def add_MLA_marker(self, x, y, materials, inner_lw=5, outer_lw=10):
        ...
        vert_box_points = [[x - inner_lw/2, y - inner_cross_length/2],
                           [x + inner_lw/2, y + inner_cross_length/2]]
        
        horz_box_points = [[x - inner_cross_length/2, y - inner_lw/2],
                           [x + inner_cross_length/2, y + inner_lw/2]]
        ...
        for i, material in enumerate(materials):
            ...
            square_points = [[x - outer_square_side_length/2 - outer_lw/2 - square_offset, y + outer_square_side_length/2 + outer_lw + square_offset],
                             [x - outer_square_side_length/2 - outer_lw/2 - square_offset, y - outer_square_side_length/2 - outer_lw/2 - square_offset],
                             [x + outer_square_side_length/2 + outer_lw/2 + square_offset, y - outer_square_side_length/2 - outer_lw/2 - square_offset],
                             [x + outer_square_side_length/2 + outer_lw/2 + square_offset, y + outer_square_side_length/2 + outer_lw/2 + square_offset],
                             [x - outer_square_side_length/2 - outer_lw - square_offset, y + outer_square_side_length/2 + outer_lw/2 + square_offset]]
            
            square = gdspy.FlexPath(square_points, outer_lw, layer=lay, datatype=dtype)
            ...
            self.Main.add(square)
            self.Main.add(vert_box)
            self.Main.add(horz_box)
        
        cutout_around_marker = gdspy.Rectangle([x - outer_square_side_length/2 - outer_lw/2 - square_offset - outer_lw, y + outer_square_side_length/2 + outer_lw + square_offset + outer_lw],
                                               [x + outer_square_side_length/2 + outer_lw/2 + square_offset + outer_lw, y - outer_square_side_length/2 - outer_lw/2 - square_offset - outer_lw])
        self.ground_plane_cutouts.add([cutout_around_marker])
    ...
    def add_initial_allignment_markers(self, x, y, cross_length=300, linewidth=5, cutout_square_size=1000):
        ...
        vert_box = gdspy.Rectangle([x - (linewidth/2), y - (cross_length/2)],
                                   [x + (linewidth/2), y + (cross_length/2)],
                                   layer=0, datatype=0)
        
        horz_box = gdspy.Rectangle([x - (cross_length/2), y - (linewidth/2)],
                                   [x + (cross_length/2), y + (linewidth/2)],
                                   layer=0, datatype=0)
        self.Main.add(vert_box)
        self.Main.add(horz_box)
        
        cutout_around_marker_rect = gdspy.Rectangle([x - (cutout_square_size/2), y - (cutout_square_size/2)],
                                                    [x + (cutout_square_size/2), y + (cutout_square_size/2)])
    ...
    def add_caliper_alignment_markers(self, x, y, rot, layer1, layer2, layer1_text, layer2_text):
        ...
        main_cross1_poly_points = [(-15.0, -100.0), (-15.0, -15.0), (-100.0, -15.0), (-100.0, 15.0), (-15.0, 15.0), (-15.0, 100.0), (15.0, 100.0), (15.0, 15.0), (100.0, 15.0), (100.0, -15.0), (15.0, -15.0), (15.0, -100.0)]
        ...
        main_cross2_poly_points = [(-11.0, -96.0), (-11.0, -11.0), (-96.0, -11.0), (-96.0, 11.0), (-11.0, 11.0), (-11.0, 96.0), (11.0, 96.0), (11.0, 11.0), (96.0, 11.0), (96.0, -11.0), (11.0, -11.0), (11.0, -96.0)]
        ...
        top_cross_poly_points = [(-73, 100), (-57, 100), (-57, 73), (-30, 73), (-30, 57), (-57, 57), (-57, 30), (-73, 30), (-73, 57), (-100, 57), (-100, 73), (-73, 73)]
        ...
        caliper = phgeom.litho_calipers(notch_size=[6,40], notch_spacing=6, num_notches=5, offset_per_notch=0.5, row_spacing=-20.0, layer1=layer1["layer"], layer2=layer2["layer"])
        ...
        rotcaliper = phgeom.litho_calipers(notch_size=[6,40], notch_spacing=6, num_notches=5, offset_per_notch=0.5, row_spacing=-20.0, layer1=layer1["layer"], layer2=layer2["layer"])    
    ...
    def add_test_linewidth_structure_box_section(self, x, y):
        """
        ...
        box_w_h = 5500
        main_square = gdspy.Rectangle([x-box_w_h/2, y-box_w_h/2], 
        ...
        outer_square_points = [[x - outer_square_side_length/2 - outer_lw/2, y + outer_square_side_length/2 + outer_lw],
        ...
        lays = [self.Aluminium["layer"],
                self.Nb_Antenna["layer"],
                self.Nb_Groundplane["layer"],
                self.SiN_dep ["layer"],
                self.IDC_Nb["layer"]]
        
        dtypes = [self.Aluminium["datatype"],
        ...
        y_pos = y
        init_x = x - (  len(materials)*(np.sum(linewidths) + (len(linewidths)-1)*gap ) + (len(materials)-1)*(big_gap) + len(materials)*(9.5/9)*label_height )/2 + ((9.5/9)*label_height + 0.125)
        ...
        for j, material, in enumerate(materials):
            ...
            for i, lw in enumerate(linewidths):
                if i==0:
                    self.Main.add(gdspy.Text(material, label_height, [x_pos-gap-(lw/2)+((2/9)*label_height), y_pos - height/2 + text_height], angle=pi/2, layer=lays[j], datatype=dtypes[j]))
                     
                self.Main.add(gdspy.Rectangle([x_pos - lw/2, y_pos - height/2], [x_pos + lw/2, y_pos + height/2], layer=lays[j], datatype=dtypes[j]))
                
                self.Main.add(gdspy.Text(str(lw) + "um", text_height, [x_pos-((5.5/9)*text_height), y_pos - height/2 - ((1/9)*text_height)], angle=3*pi/2, layer=lays[j], datatype=dtypes[j]))
    ...
    def add_test_crossover_structure_box_section(self, x, y, Main_config_file_dict, rot=0.0):
        ...
        self.silicon_nitride_positives.add(gdspy.Rectangle([x-box_w_h/2, y-box_w_h/2], 
                                                         [x+box_w_h/2, y+box_w_h/2],
                                                         **self.SiN_dep))
        ...
        pad_left_poly_points = [[0, pad_back_width/2],
        ...
        pad_left_connect = [(pad_left_poly_points[2][0] + pad_left_poly_points[3][0])/2, (pad_left_poly_points[2][1] + pad_left_poly_points[3][1])/2]
        pad_left_center = [(pad_back_length + 0)/2, 0]
        
        pad_left_cutout_poly_points = [[pad_left_center[0] - (pad_cutout_length/2), pad_left_center[1] - (pad_cutout_width/2)],
                                       [pad_left_center[0] - (pad_cutout_length/2), pad_left_center[1] + (pad_cutout_width/2)],
                                       [pad_left_center[0] + (pad_cutout_length/2), pad_left_center[1] + (pad_cutout_width/2)],
                                       [pad_left_center[0] + (pad_cutout_length/2), pad_left_center[1] - (pad_cutout_width/2)]
                                       ]
        
        pad_right_poly_points = [[0, pad_back_width/2],
                                 [-pad_back_length, pad_back_width/2],
                                 [-pad_back_length - pad_taper_length, conect_linewidth/2],
                                 [-pad_back_length - pad_taper_length, -conect_linewidth/2],
                                 [-pad_back_length, -pad_back_width/2],
        ...
        pad_right_connect = [(pad_right_poly_points[2][0] + pad_right_poly_points[3][0])/2, (pad_right_poly_points[2][1] + pad_right_poly_points[3][1])/2]
        pad_right_center = [(-pad_back_length - 0)/2, 0]
        
        pad_right_cutout_poly_points = [[pad_right_center[0] - (pad_cutout_length/2), pad_right_center[1] - (pad_cutout_width/2)],
        ...
        for set_no in range(3):
            ...
            cross_connections = self.add_filter_bank_ring_overlap_and_get_conections(cross_xy[0], cross_xy[1], cross_xy[0] + 100, cross_xy[1], 0, cross_angle + rot, Main_config_file_dict)
            ground_under_cross = gdspy.Rectangle([cross_xy[0] - (ground_under_crossover_w_h/2), cross_xy[1] - (ground_under_crossover_w_h/2)],
            ...
            if rot<pi/4:
                cross_connects = [cross_connections["inner_conect_1"],
                                  cross_connections["outer_conect_1"],
                                  cross_connections["inner_conect_0"],
                                  cross_connections["outer_conect_0"]
                                  ]
            elif rot<pi/2:
                cross_connects = [cross_connections["outer_conect_0"],
                                  cross_connections["outer_conect_1"],
                                  cross_connections["inner_conect_0"],
                                  cross_connections["inner_conect_1"]
                                  ]
            elif rot<3*pi/4:
                cross_connects = [cross_connections["inner_conect_0"],
                                  cross_connections["inner_conect_1"],
                                  cross_connections["outer_conect_0"],
                                  cross_connections["outer_conect_1"]
                                  ]
            ...
            for i in range(4):
                ...
                pad1_path = gdspy.FlexPath([[pad_connect[0], pad_connect[1]],
                                            ...
                                            ], 5, corners="circular bend", bend_radius=50, gdsii_path=True, **self.Nb_Antenna)
    ...
    def make_DC_structure_pads_and_meander(self, x, y, rot, DC_structure_material):
        ...
        pad_left = gdspy.Rectangle([pad_center_x_left - (pad_size/2), -(pad_size/2)],
                                    [pad_center_x_left + (pad_size/2), +(pad_size/2)])
        
        pad_right = gdspy.Rectangle([pad_center_x_right - (pad_size/2), -(pad_size/2)],
                                    [pad_center_x_right + (pad_size/2), +(pad_size/2)])
        
        
        meander_points = [[-(total_width_meander/2), -meander_bend_height_bot],
    ...
    def add_test_DC_structure_box_section(self, x, y, DC_structure_material, BL_text=""):
        ...
        centers_and_rots = [[0, pad_and_meander_offset, 0],
    ...
    def add_test_chip_quad_groundplane_edge_cuts(self, test_chip_quad_center_xy, test_chip_quad_width, test_chip_quad_height, top_mid_gap=0, bot_mid_gap=0, right_mid_gap=0, left_mid_gap=0):
        """
    ...
    # Adding the top and bot edge cuts
        for i in range(int(no_of_edge_cuts_horizontal/2) + 1):
            ...
            if edge_cut_offset_from_center >= (top_mid_gap/2):
                # right edge cut
                edge_cut_center_xy = [test_chip_quad_center_xy[0] + edge_cut_offset_from_center,
                                      test_chip_quad_center_xy[1] + (test_chip_quad_height/2) - (edge_cut_width/2)]
                
                top_edge_cut_rect_right = gdspy.Rectangle([edge_cut_center_xy[0] - (edge_cut_length/2), edge_cut_center_xy[1] - (edge_cut_width/2)],
                ...
                # left edge cut
                edge_cut_center_xy = [test_chip_quad_center_xy[0] + (-1*edge_cut_offset_from_center),
                                      test_chip_quad_center_xy[1] + (test_chip_quad_height/2) - (edge_cut_width/2)]
                
                top_edge_cut_rect_left = gdspy.Rectangle([edge_cut_center_xy[0] - (edge_cut_length/2), edge_cut_center_xy[1] - (edge_cut_width/2)],
                                                         [edge_cut_center_xy[0] + (edge_cut_length/2), edge_cut_center_xy[1] + (edge_cut_width/2)],
                                                          **self.Nb_Groundplane)
            ...
            if edge_cut_offset_from_center >= (bot_mid_gap/2):
                ...
                bot_edge_cut_rect_right = gdspy.Rectangle([edge_cut_center_xy[0] - (edge_cut_length/2), edge_cut_center_xy[1] - (edge_cut_width/2)],
                                                          [edge_cut_center_xy[0] + (edge_cut_length/2), edge_cut_center_xy[1] + (edge_cut_width/2)],
                                                          **self.Nb_Groundplane)
                ...
                bot_edge_cut_rect_left = gdspy.Rectangle([edge_cut_center_xy[0] - (edge_cut_length/2), edge_cut_center_xy[1] - (edge_cut_width/2)],
                                                         [edge_cut_center_xy[0] + (edge_cut_length/2), edge_cut_center_xy[1] + (edge_cut_width/2)],
                                                         **self.Nb_Groundplane)
    ...
    # Adding the left and right edge cuts
        for i in range(int(no_of_edge_cuts_vertical/2) + 1):
            ...
            if edge_cut_offset_from_center >= (right_mid_gap/2):
                # top edge cut
                edge_cut_center_xy = [test_chip_quad_center_xy[0] + (test_chip_quad_width/2) - (edge_cut_width/2),
                                      test_chip_quad_center_xy[1] + edge_cut_offset_from_center]
                
                right_edge_cut_rect_top = gdspy.Rectangle([edge_cut_center_xy[0] - (edge_cut_width/2), edge_cut_center_xy[1] - (edge_cut_length/2)],
                ...
                right_edge_cut_rect_bot = gdspy.Rectangle([edge_cut_center_xy[0] - (edge_cut_width/2), edge_cut_center_xy[1] - (edge_cut_length/2)],
                                                          [edge_cut_center_xy[0] + (edge_cut_width/2), edge_cut_center_xy[1] + (edge_cut_length/2)],
                                                          **self.Nb_Groundplane)
            ...
            if edge_cut_offset_from_center >= (left_mid_gap/2):
                ...
                left_edge_cut_rect_top = gdspy.Rectangle([edge_cut_center_xy[0] - (edge_cut_width/2), edge_cut_center_xy[1] - (edge_cut_length/2)],
                                                          [edge_cut_center_xy[0] + (edge_cut_width/2), edge_cut_center_xy[1] + (edge_cut_length/2)],
                                                          **self.Nb_Groundplane)
                ...
                left_edge_cut_rect_bot = gdspy.Rectangle([edge_cut_center_xy[0] - (edge_cut_width/2), edge_cut_center_xy[1] - (edge_cut_length/2)],
                                                          [edge_cut_center_xy[0] + (edge_cut_width/2), edge_cut_center_xy[1] + (edge_cut_length/2)],
                                                          **self.Nb_Groundplane)
    ...
    def add_test_chip_quad_tabbed_dicing_line(self, test_chip_quad_center_xy, test_chip_quad_width, test_chip_quad_height, tab_position="left", corner_overrun_length=200):
        """
        ...
        right_rect = gdspy.Rectangle([bot_right_corner[0], bot_right_corner[1] - linewidth - corner_overrun_length],
        ...
        top_rect = gdspy.Rectangle([top_left_corner[0] - linewidth - corner_overrun_length, top_left_corner[1]],
        ...
        bot_rect = gdspy.Rectangle([bot_left_corner[0] - linewidth - corner_overrun_length, bot_left_corner[1] - linewidth],
        ...
        path_middle = gdspy.FlexPath([[middle_left_center[0], middle_left_center[1] - tab_length/2],
                                      [middle_left_center[0], middle_left_center[1] + tab_length/2]
                                      ],
                                     linewidth, ends="round", **self.Tab_dicing_line)
        ...
        for i in range( int(np.round( (test_chip_quad_height/2)/(tab_length + tab_gap_length + linewidth), 0 )) + 1 ):
            ...
            if (tab_length/2 + i*(tab_length + 2*tab_gap_length)) > test_chip_quad_height/2:
                ...
                edge_tab = gdspy.FlexPath([[middle_left_center[0], middle_left_center[1] - tab_length/2 + i*(tab_length + 2*tab_gap_length)],
                                           [middle_left_center[0], top_left_corner[1] + linewidth + corner_overrun_length]
                                           ],
                                           linewidth, **self.Tab_dicing_line)
                edge_tab.rotate(rotation, [center_x, center_y])
                self.Main.add(edge_tab)
                
                cap = gdspy.Round([middle_left_center[0], middle_left_center[1] - tab_length/2 + i*(tab_length + 2*tab_gap_length)], linewidth/2, initial_angle=0, final_angle=-pi, **self.Tab_dicing_line)
                cap.rotate(rotation, [center_x, center_y])
                self.Main.add(cap)
                
                edge_tab = gdspy.FlexPath([[middle_left_center[0], bot_left_corner[1] - linewidth - corner_overrun_length],
    ...
    def add_text_under_horn_in_test_chip_quad(self, text_string, horn_center_x, horn_center_y):
        ...
        under_text = gdspy.Text(text_string, text_size, position=[horn_center_x + x_offset_from_horn, horn_center_y + y_offset_from_horn], **self.Aluminium)
        under_text.translate(-abs(under_text.get_bounding_box()[0][0]-under_text.get_bounding_box()[1][0])/2, 0)
        self.Main.add(under_text)
        self.ground_plane_cutouts.add(gdspy.Rectangle(under_text.get_bounding_box()[0], under_text.get_bounding_box()[1], **self.Nb_Groundplane))
    ...
    def get_config_excel(self, file_name, sheet_name, path="config_files\\"):
        ...
        try:
            ...
            print("Config file '" + file_name + "' not found.")
            return
        except Exception:
    ...
    def create_miter_join(self, p0, v0, p1, v1, p2, w):
        ...
        return [[p0[0] + (w/2)*cos(angle0 + pi), p0[1] + (w/2)*sin(angle0 + pi)], [p1[0] + (w/2)*cos(angle1), p1[1] + (w/2)*sin(angle1)]]
    ...
    def add_antena(self, x,y, rot, Main_config_file_dict, add_grnd_cutout=True, add_SiN_dep_cutout=True, add_backside_check=True):
        ...
        cicle_cutout_radius = distance_from_center + straight_height + taper_height
        
        antena_geometry = [[(top_conect_width/2) + x,   -(taper_height+straight_height+distance_from_center) + y],
                           ...
                           [(base_width/2) + x,         -(distance_from_center) + y],
                           [(base_width/2) + x,         -(straight_height+distance_from_center)  + y]] # this is the shape of the antenna with the point at the bottom
    
        ant_bot = gdspy.Polygon(antena_geometry, **self.Nb_Antenna) # this defines the bottom, right, top and left antennas as the same bottom antenna shape
        ...
        ant_top.rotate(self.deg_to_rad(rot + 180), (x, y))  # this also rotates the top, left and right antennas which are all the same as the bottom to form 4 antennas orthogonal to each other
        ...
        if add_grnd_cutout:
            grnd_cutout_circle = gdspy.Round([x, y], cicle_cutout_radius, **self.Nb_Groundplane)
            self.ground_plane_cutouts.add(grnd_cutout_circle)
        if add_SiN_dep_cutout:
            SiN_dep_cutout_circle = gdspy.Round([x, y], cicle_cutout_radius, **self.SiN_dep)
    ...
    def add_4_KID_outers_and_arms_new(self, x, y, rel_kid_positions, KID_Nos, f0s, IDC_and_CC_function, Main_config_file_dict, add_grnd_cutout=True, add_SiN_dep_dielectric_around=True, add_SiO_cutout=True, add_SiN_membrane_cutout=True, add_backside_check=True):
        """
        ...
        for k in range(4):
            ...
            self.add_kid(kid_x, kid_y, rot_angles[k], f0s[k], IDC_and_CC_function, Main_config_file_dict, mirror=to_mirror[k])
        ...
        #Adding the KID number Text
            ...
            KID_text_label = gdspy.Text(str(KID_Nos[k]), text_size, (kid_x + x_sign*text_x_offset , kid_y + y_sign*text_y_offset), **self.Aluminium)
            underline = gdspy.Rectangle([kid_x + x_sign*text_x_offset , kid_y + y_sign*text_y_offset], [kid_x + x_sign*text_x_offset + undln_len , kid_y + y_sign*text_y_offset + text_underline_height], **self.Aluminium)
        ...
        if add_SiN_dep_dielectric_around:
            ...
            dielectric_around_box = gdspy.Rectangle([x - dielectric_around_width/2, y - dielectric_around_height/2],
                                                    [x + dielectric_around_width/2, y + dielectric_around_height/2], **self.SiN_dep)
            self.silicon_nitride_positives.add(dielectric_around_box)
            
        
        return
    
    def add_kid(self, x, y, rot_angle, f0, IDC_and_CC_function, Main_config_file_dict, mirror=False, add_grnd_cutout=True, add_SiN_dep_dielectric_around=True, add_SiO_cutout=True, add_SiN_membrane_cutout=True, add_backside_check=True):
        """
        ...
        meander_path_points = [[(meander_bot_width/2) - meander_right_width_1 + meander_right_width_2, meander_lw + meander_right_height_1 + meander_right_height_2 + meander_right_height_3],
                               [(meander_bot_width/2) - meander_right_width_1 + meander_right_width_2, (meander_lw/2) + meander_right_height_1 + meander_right_height_2],
                               ...
                               [-(meander_bot_width/2) - meander_left_width_1 + meander_left_width_2, (meander_lw/2) + meander_left_height_1 + meander_left_height_2],
                               [-(meander_bot_width/2) - meander_left_width_1 + meander_left_width_2, meander_lw + meander_left_height_1 + meander_left_height_2 + meander_left_height_3]
                               ]
        #Making the meander ant pad
        ant_pad_box_width =config["ant_pad_box_width"]# 5
        ant_pad_box_height = config["ant_pad_box_height"]#10
        
        ant_pad_box_points = [[-ant_pad_box_width/2, 0],
        ...
        frame_left_points = [[frame_left_start_x, frame_start_y + frame_bot_lw],
        ...
        frame_right_points = [[frame_right_start_x, frame_start_y + frame_bot_lw],
        ...
        meander_cover_box_right_points = [[meander_path_points[0][0] - (frame_meander_cover_box_width/2), meander_path_points[0][1] + frame_bot_lw],
                                          [meander_path_points[0][0] - (frame_meander_cover_box_width/2), meander_path_points[0][1] + frame_bot_lw - frame_meander_cover_box_height],
                                          [meander_path_points[0][0] + (frame_meander_cover_box_width/2), meander_path_points[0][1] + frame_bot_lw - frame_meander_cover_box_height],
                                          [meander_path_points[0][0] + (frame_meander_cover_box_width/2), meander_path_points[0][1] + frame_bot_lw]
                                         ]
        
        meander_cover_box_left_points = [[meander_path_points[-1][0] - (frame_meander_cover_box_width/2), meander_path_points[-1][1] + frame_bot_lw],
                                          [meander_path_points[-1][0] - (frame_meander_cover_box_width/2), meander_path_points[-1][1] + frame_bot_lw - frame_meander_cover_box_height],
                                          [meander_path_points[-1][0] + (frame_meander_cover_box_width/2), meander_path_points[-1][1] + frame_bot_lw - frame_meander_cover_box_height],
        ...
        cutout_width = (frame_right_start_x + frame_bot_right_width + cutout_right_offset) - (frame_left_start_x - frame_bot_left_width - cutout_left_offset)
        ...
        grnd_plane_meander_cutout_poly_points = [[-grnd_plane_cutout_width/2, grnd_plane_cutout_start_height],
        ...
        SiN_dep_cutout_poly_points = [[-SiN_dep_cutout_width/2, SiN_dep_cutout_start_height],
        ...
        SiO_cutout_poly_points = [[-SiO_stepdown_cutout_width/2, SiO_cutout_start_height + SiO_stepdown_cutout_height],
        ...
        SiN_membrane_cutout_poly_points = [[-SiN_membrane_stepdown_cutout_width/2, SiN_membrane_cutout_start_height + SiN_membrane_stepdown_cutout_height],
        ...
        backside_check_cover_poly_points = [[-backside_check_cover_width/2, backside_check_cutout_start_height],
    ...
    #Adding the meander
        ...
        meander_path = gdspy.FlexPath(new_meander_path_points, meander_lw, corners="circular bend", bend_radius=meander_corner_bend_radius, **self.Aluminium)
        self.Main.add(meander_path)
    ...
    #Adding the meander ant overlap box
        ...
        ant_pad_box = gdspy.Polygon(new_ant_pad_box_points, **self.Aluminium)
    ...
    #Adding the meander frame overlap boxes
        ...
        meander_cover_box_right = gdspy.Polygon(new_meander_cover_box_right_points, **self.IDC_Nb)
        ...
        meander_cover_box_left = gdspy.Polygon(new_meander_cover_box_left_points, **self.IDC_Nb)
    ...
    #Adding the frame left and frame right
        ...
        frame_left_poly = gdspy.Polygon(new_frame_left_points, **self.IDC_Nb)
        ...
        frame_right_poly = gdspy.Polygon(new_frame_right_points, **self.IDC_Nb)
        self.Main.add(frame_right_poly)
        
    #Adding the coupler frame
        coupler_frame_points = [[coupler_frame_start_x - (coupler_frame_left_lw/2), coupler_frame_start_y],
                                ...
                                [coupler_frame_start_x - (coupler_frame_left_lw/2) + CCL, coupler_frame_start_y + coupler_frame_left_height - coupler_frame_top_lw],
        ...
        coupler_frame_poly = gdspy.Polygon(new_coupler_frame_points, **self.IDC_Nb)
        self.Main.add(coupler_frame_poly)
        
    #Adding the coupler arm
        coupler_arm_points = [[coupler_frame_start_x - (coupler_frame_left_lw/2) - left_coupler_frame_to_feed_distance, coupler_frame_start_y + coupler_frame_left_height + coupler_gap],
                              [coupler_frame_start_x - (coupler_frame_left_lw/2) + CCL, coupler_frame_start_y + coupler_frame_left_height + coupler_gap],
                              [coupler_frame_start_x - (coupler_frame_left_lw/2) + CCL, coupler_frame_start_y + coupler_frame_left_height + coupler_gap + coupler_lw],
                              [coupler_frame_start_x - (coupler_frame_left_lw/2) - left_coupler_frame_to_feed_distance, coupler_frame_start_y + coupler_frame_left_height + coupler_gap + coupler_lw]
                              ]
        if mirror:
    ...
    #Adding the IDC arms
        for i in range(0, No_of_arms, 2):
            right_arm = gdspy.Rectangle([arm_start_x_right_side, arm_start_y_right_side -(IDC_arm_lw/2) + (i*(IDC_arm_gap + IDC_arm_lw))],
                                        [arm_start_x_right_side - IDCLs[-(i+1)], arm_start_y_right_side +(IDC_arm_lw/2) + (i*(IDC_arm_gap + IDC_arm_lw))], **self.IDC_Nb)
            ...
            left_arm = gdspy.Rectangle([arm_start_x_left_side, arm_start_y_left_side -(IDC_arm_lw/2) + (i*(IDC_arm_gap + IDC_arm_lw))],
                                        [arm_start_x_left_side + IDCLs[-(i+2)], arm_start_y_left_side +(IDC_arm_lw/2) + (i*(IDC_arm_gap + IDC_arm_lw))], **self.IDC_Nb)
    ...
    #Adding the Trim arms
        right_trim_arm = gdspy.Rectangle([arm_start_x_right_side, trim_arm_start_y_right_side],
                                          [arm_start_x_right_side - trim_arm_length_right_side, trim_arm_start_y_right_side + trim_arm_lw], **self.IDC_Nb)
        ...
        left_trim_arm = gdspy.Rectangle([arm_start_x_left_side, trim_arm_start_y_left_side],
                                          [arm_start_x_left_side + trim_arm_length_left_side, trim_arm_start_y_left_side + trim_arm_lw], **self.IDC_Nb)
    ...
    #Adding the cutout to the groundplane
        if add_grnd_cutout:
            if mirror:
                ...
                new_grnd_plane_meander_cutout_poly_points = self.rotate_and_move_points_list(new_grnd_plane_meander_cutout_poly_points, rot_angle, x, y)
            else:
                new_grnd_plane_meander_cutout_poly_points = self.rotate_and_move_points_list(grnd_plane_meander_cutout_poly_points, rot_angle, x, y)
    ...
    #Adding the backside check covers
        if add_backside_check:
            if mirror:
                ...
                new_backside_check_cover_poly_points = self.rotate_and_move_points_list(new_backside_check_cover_poly_points, rot_angle, x, y)
            else:
    ...
    def add_Lo_pass_filters(self, x, y, inner_ring_line_width, inner_ring_radius, init_angle, direction, Main_config_file_dict):
        ...
        arm_linewidths = [Lo_pass_arm1_linewidth, Lo_pass_arm2_linewidth, Lo_pass_arm3_linewidth, Lo_pass_arm4_linewidth, Lo_pass_arm5_linewidth]
        ...
        offset_arc_angles = [Lo_pass_arm1_offset_arc_angle, Lo_pass_arm2_offset_arc_angle, Lo_pass_arm3_offset_arc_angle, Lo_pass_arm4_offset_arc_angle, Lo_pass_arm5_offset_arc_angle]
        ...
        for i in range(5):
            ...
            arm_geom_points = [[(sign)*arm_linewidths[i]/2, 0],
                               ...
                               [(-sign)*arm_linewidths[i]/2 + (-sign)*arm_lengths[i], arm_heights[i] + arm_linewidths[i] + (inner_ring_line_width/2)],
            ...
            via_points = [[(-sign)*arm_linewidths[i]/2 + (-sign)*arm_lengths[i] + (sign)*via_overlap_length, arm_heights[i] + arm_linewidths[i] + (inner_ring_line_width/2)],
                          [(-sign)*arm_linewidths[i]/2 + (-sign)*arm_lengths[i] + (sign)*via_overlap_length, arm_heights[i] + (inner_ring_line_width/2)],
                          [(-sign)*arm_linewidths[i]/2 + (-sign)*arm_lengths[i] + (-sign)*via_extend_length, arm_heights[i] + (inner_ring_line_width/2)],
                          [(-sign)*arm_linewidths[i]/2 + (-sign)*arm_lengths[i] + (-sign)*via_extend_length, arm_heights[i] + arm_linewidths[i] + (inner_ring_line_width/2)]]
    ...
    def add_Hi_pass_filters(self, x, y, inner_ring_line_width, inner_ring_radius, init_angle, direction, Main_config_file_dict):
        ...
        arm_linewidths = [Hi_pass_arm1_linewidth, Hi_pass_arm2_linewidth, Hi_pass_arm3_linewidth, Hi_pass_arm4_linewidth, Hi_pass_arm5_linewidth]
        ...
        offset_arc_angles = [Hi_pass_arm1_offset_arc_angle, Hi_pass_arm2_offset_arc_angle, Hi_pass_arm3_offset_arc_angle, Hi_pass_arm4_offset_arc_angle, Hi_pass_arm5_offset_arc_angle]
        offset_arc_cumsum_angles = np.cumsum(offset_arc_angles)
    ...
    def add_combiner_section_and_get_conect_point(self, x, y, rot, outer_ring_conection_gap, outer_ring_linewidth, Main_config_file_dict, combiner_type="90GHZ", mirror_combiner=False):
        """
        ...
        if combiner_type =="90GHZ":
            ...
            print("'combiner_type' argument should be either '90GHZ' or '150GHZ'")
    ...
    # Making First section
        ...
        first_section_top_points = [(-outer_ring_linewidth/2, outer_ring_conection_gap/2),
        ...
        first_section_bot_points = [(-outer_ring_linewidth/2, -outer_ring_conection_gap/2),
                           (distance_from_outer_ring, -outer_ring_conection_gap/2),
                           (distance_from_outer_ring, -first_section_height/2),
                           (distance_from_outer_ring + first_section_width, -first_section_height/2),
                           (distance_from_outer_ring + first_section_width, -first_section_height/2 + first_section_back_height)]
        ...
        if mirror_combiner:
            ...
            first_section_top = gdspy.FlexPath(mirrored_first_section_top_points, first_linewidth, corners=self.create_miter_join, **self.Nb_Antenna)
            first_section_bot = gdspy.FlexPath(mirrored_first_section_bot_points, first_linewidth, corners=self.create_miter_join, **self.Nb_Antenna)
        else:
    ...
    # Making second section
        ...
        second_section_top_points = [second_section_top_first_point,
                                     ...
                                     [second_section_top_first_point[0]+second_section_top_width1+second_section_top_width2, second_section_top_first_point[1]-second_section_top_height1],
                                     [second_section_top_first_point[0]+second_section_top_width1+second_section_top_width2, second_section_top_first_point[1]-second_section_top_height1+second_section_top_height2],
                                     [second_section_top_first_point[0]+second_section_top_width1+second_section_top_width2+second_section_top_width3, second_section_top_first_point[1]-second_section_top_height1+second_section_top_height2],
                                     [second_section_top_first_point[0]+second_section_top_width1+second_section_top_width2+second_section_top_width3, second_section_top_first_point[1]-second_section_top_height1+second_section_top_height2-second_section_top_height3],
                                     [second_section_top_first_point[0]+second_section_top_width1+second_section_top_width2+second_section_top_width3+second_section_top_width4, second_section_top_first_point[1]-second_section_top_height1+second_section_top_height2-second_section_top_height3],
                                     [second_section_top_first_point[0]+second_section_top_width1+second_section_top_width2+second_section_top_width3+second_section_top_width4, second_section_top_first_point[1]-second_section_top_height1+second_section_top_height2-second_section_top_height3+second_section_top_height4],
                                     [second_section_top_first_point[0]+second_section_top_width1+second_section_top_width2+second_section_top_width3+second_section_top_width4+second_section_top_width5, second_section_top_first_point[1]-second_section_top_height1+second_section_top_height2-second_section_top_height3+second_section_top_height4]]
        
        if mirror_combiner:
            mirrored_second_section_top_points = self.mirror_points_around_xaxis(second_section_top_points)
            second_section_top = gdspy.FlexPath(mirrored_second_section_top_points, second_section_linewidth, corners=self.create_miter_join, **self.Nb_Antenna)
        else:
            second_section_top = gdspy.FlexPath(second_section_top_points, second_section_linewidth, corners=self.create_miter_join, **self.Nb_Antenna)
        ...
        second_section_bot_points = [second_section_bot_first_point,
                                     ...
                                     [second_section_bot_first_point[0]+second_section_bot_width1+second_section_bot_width2, second_section_bot_first_point[1]+second_section_bot_height1],
                                     [second_section_bot_first_point[0]+second_section_bot_width1+second_section_bot_width2, second_section_bot_first_point[1]+second_section_bot_height1-second_section_bot_height2],
                                     
                                     [second_section_bot_first_point[0]+second_section_bot_width1+second_section_bot_width2+second_section_bot_width3, second_section_bot_first_point[1]+second_section_bot_height1-second_section_bot_height2],
                                     [second_section_bot_first_point[0]+second_section_bot_width1+second_section_bot_width2+second_section_bot_width3, second_section_bot_first_point[1]+second_section_bot_height1-second_section_bot_height2+second_section_bot_height3],
                                     
                                     [second_section_bot_first_point[0]+second_section_bot_width1+second_section_bot_width2+second_section_bot_width3+second_section_bot_width4, second_section_bot_first_point[1]+second_section_bot_height1-second_section_bot_height2+second_section_bot_height3],
                                     [second_section_bot_first_point[0]+second_section_bot_width1+second_section_bot_width2+second_section_bot_width3+second_section_bot_width4, second_section_bot_first_point[1]+second_section_bot_height1-second_section_bot_height2+second_section_bot_height3-second_section_bot_height4],
                                     [second_section_bot_first_point[0]+second_section_bot_width1+second_section_bot_width2+second_section_bot_width3+second_section_bot_width4+second_section_bot_width5, second_section_bot_first_point[1]+second_section_bot_height1-second_section_bot_height2+second_section_bot_height3-second_section_bot_height4]]
        
        if mirror_combiner:
            mirrored_second_section_bot_points = self.mirror_points_around_xaxis(second_section_bot_points)
            second_section_bot = gdspy.FlexPath(mirrored_second_section_bot_points, second_section_linewidth, corners=self.create_miter_join, **self.Nb_Antenna)
        else:
            second_section_bot = gdspy.FlexPath(second_section_bot_points, second_section_linewidth, corners=self.create_miter_join, **self.Nb_Antenna)
        ...
        start_of_second_section_vertical_points = [[first_section_top_points[-1][0]-first_linewidth/2+start_of_second_section_vertical_linewidth/2, first_section_top_points[-1][1]],
                                                   [first_section_bot_points[-1][0]-first_linewidth/2+start_of_second_section_vertical_linewidth/2, first_section_bot_points[-1][1]]
                                                   ]
        if mirror_combiner:
            mirrored_start_of_second_section_vertical_points = self.mirror_points_around_xaxis(start_of_second_section_vertical_points)
            start_of_second_section_vertical = gdspy.FlexPath(mirrored_start_of_second_section_vertical_points, start_of_second_section_vertical_linewidth, **self.Nb_Antenna)
        else:
            start_of_second_section_vertical = gdspy.FlexPath(start_of_second_section_vertical_points, start_of_second_section_vertical_linewidth, **self.Nb_Antenna)
        ...
        end_of_second_section_vertical_points = [[second_section_top_points[-1][0], second_section_top_points[-1][1] + second_section_linewidth/2],
                                                 [second_section_top_points[-1][0] + end_of_second_section_vertical_linewidth, second_section_top_points[-1][1] - second_section_linewidth/2 + third_section_linewidth],
                                                 [second_section_bot_points[-1][0] + end_of_second_section_vertical_linewidth, second_section_bot_points[-1][1] + second_section_linewidth/2 - third_section_linewidth],
    ...
    # Making third section
        ...
        third_section_start_points = [end_of_second_section_vertical_points[1][0], end_of_second_section_vertical_points[1][1]-third_section_linewidth/2]
        third_section_end_points = [end_of_second_section_vertical_points[2][0], end_of_second_section_vertical_points[2][1]+third_section_linewidth/2]
        
        third_section_points = [third_section_start_points,
                                ...
                                [third_section_start_points[0]+third_section_width1+third_section_width2, third_section_start_points[1]-third_section_height1],
                                [third_section_start_points[0]+third_section_width1+third_section_width2, third_section_start_points[1]-third_section_height1+third_section_height2],
                                [third_section_start_points[0]+third_section_width1+third_section_width2+third_section_width3, third_section_start_points[1]-third_section_height1+third_section_height2],
                                
                                [third_section_end_points[0]+third_section_width1+third_section_width2+third_section_width3, third_section_end_points[1]+third_section_height1-third_section_height2],
                                [third_section_end_points[0]+third_section_width1+third_section_width2, third_section_end_points[1]+third_section_height1-third_section_height2],
                                [third_section_end_points[0]+third_section_width1+third_section_width2, third_section_end_points[1]+third_section_height1],
        ...
        if mirror_combiner:
            ...
            third_section = gdspy.FlexPath(mirrored_third_section_points, third_section_linewidth, corners=self.create_miter_join, **self.Nb_Antenna)
        else:
        ...
        conection_to_kid_path_start_path_points = [[third_section_points[5][0] - third_section_linewidth/2, third_section_points[5][1] + third_section_linewidth/2 - conection_to_kid_path_linewidth/2],
                                                   [third_section_points[5][0] - third_section_linewidth/2 + conection_to_kid_path_start_piece_length, third_section_points[5][1] + third_section_linewidth/2 - conection_to_kid_path_linewidth/2]]
        
        conection_to_kid_path_start_path_points = [[third_section_points[5][0] + third_section_linewidth/2 - conection_to_kid_path_linewidth/2, third_section_points[5][1] - third_section_linewidth/2],
                                                   [third_section_points[5][0] + third_section_linewidth/2 - conection_to_kid_path_linewidth/2, third_section_points[5][1] - third_section_linewidth/2 + conection_to_kid_path_start_piece_length]
                                                   ]
        
        if mirror_combiner:
            mirrored_conection_to_kid_path_start_path_points = self.mirror_points_around_xaxis(conection_to_kid_path_start_path_points)
            conection_to_kid_path_start = gdspy.FlexPath(mirrored_conection_to_kid_path_start_path_points, conection_to_kid_path_linewidth, **self.Nb_Antenna)
        else:
            conection_to_kid_path_start = gdspy.FlexPath(conection_to_kid_path_start_path_points, conection_to_kid_path_linewidth, **self.Nb_Antenna)
    ...
    # Making the meander section and connect to frame
        ...
        for i in range(no_of_full_straights-1):
            direction*=-1
            meander_points.append([meander_offset_x + i*meander_gap_spacing, direction*meander_height/2 + direction*meander_linewidth/2])
            meander_points.append([meander_offset_x + i*meander_gap_spacing, -direction*meander_height/2 - direction*meander_linewidth/2])
        ...
        self.Main.add(meander)
        
        
        meander_to_frame_box_size = config["meander_to_frame_box_size"]
        
        meander_to_frame_box = gdspy.Rectangle([meander_points[-1][0] - (meander_to_frame_box_size/2), meander_points[-1][1] - (meander_to_frame_box_size/2) + (meander_linewidth/2)],
                                               [meander_points[-1][0] + (meander_to_frame_box_size/2), meander_points[-1][1] + (meander_to_frame_box_size/2) + (meander_linewidth/2)], **self.Nb_Antenna)
        if mirror_combiner:
        ...
        connect_meander_to_frame_points = [[meander_points[-1][0], meander_points[-1][1]],
                                            ...
                                            [meander_points[-1][0] + meander_conect_bot_width + meander_conect_final_width, -meander_height/2 - meander_linewidth/2 + meander_conect_right_height]]
        if mirror_combiner:
            mirrored_connect_meander_to_frame_points = self.mirror_points_around_xaxis(connect_meander_to_frame_points)
            connect_meander_to_frame = gdspy.FlexPath(mirrored_connect_meander_to_frame_points, meander_conect_linewidth, corners=self.create_miter_join, **self.Nb_Antenna)
        else:
            connect_meander_to_frame = gdspy.FlexPath(connect_meander_to_frame_points, meander_conect_linewidth, corners=self.create_miter_join, **self.Nb_Antenna)
        ...
        meander_fork_start_xy = [connect_meander_to_frame_points[2][0] - meander_linewidth/2, connect_meander_to_frame_points[2][1] + meander_last_fork_start_height + meander_linewidth/2 + meander_last_fork_linewdith/2]
        
        meander_last_fork_points = [meander_fork_start_xy,
                                    [meander_fork_start_xy[0] - meander_last_fork_wdith - meander_last_fork_linewdith/2, meander_fork_start_xy[1]],
                                    [meander_fork_start_xy[0] - meander_last_fork_wdith - meander_last_fork_linewdith/2, meander_fork_start_xy[1] + meander_last_fork_height + meander_last_fork_linewdith/2]]
        if mirror_combiner:
            mirrored_meander_last_fork_points = self.mirror_points_around_xaxis(meander_last_fork_points)
            meander_last_fork = gdspy.FlexPath(mirrored_meander_last_fork_points, meander_last_fork_linewdith, corners=self.create_miter_join, **self.Nb_Antenna)
        else:
            meander_last_fork = gdspy.FlexPath(meander_last_fork_points, meander_last_fork_linewdith, corners=self.create_miter_join, **self.Nb_Antenna)
        ...
        meander_last_fork_top_box = gdspy.Rectangle([meander_last_fork_points[-1][0] - meander_last_fork_top_box_size/2, meander_last_fork_points[-1][1]],
                                                    [meander_last_fork_points[-1][0] + meander_last_fork_top_box_size/2, meander_last_fork_points[-1][1] + meander_last_fork_top_box_size], **self.Nb_Antenna)
        if mirror_combiner:
        ...
        meander_last_fork_via_box = gdspy.Rectangle([meander_last_fork_points[-1][0] - meander_last_fork_via_box_size_x/2, meander_last_fork_points[-1][1] + meander_last_fork_via_box_y_offset],
                                                    [meander_last_fork_points[-1][0] + meander_last_fork_via_box_size_x/2, meander_last_fork_points[-1][1] + meander_last_fork_via_box_y_offset + meander_last_fork_via_box_size_y], **self.SiN_dep)
        if mirror_combiner:
    ...
    def add_filter_bank_ring_overlap_and_get_conections(self, x, y, ant_center_x, ant_center_y, overlap_no, rot, Main_config_file_dict):
        ...
        conections_rotated_to_quadrant_1 = [self.rotate(ant_center_x, ant_center_y, conect_half_conect_left[0], conect_half_conect_left[1], (overlap_no*-pi/2)),
        ...
        conections_dict = {}
        conections_dict["inner_conect_0"] = self.rotate(ant_center_x, ant_center_y, conections_sorted_list[0][0], conections_sorted_list[0][1], (overlap_no*pi/2))
        conections_dict["outer_conect_0"] = self.rotate(ant_center_x, ant_center_y, conections_sorted_list[1][0], conections_sorted_list[1][1], (overlap_no*pi/2))
        conections_dict["inner_conect_1"] = self.rotate(ant_center_x, ant_center_y, conections_sorted_list[2][0], conections_sorted_list[2][1], (overlap_no*pi/2))
        conections_dict["outer_conect_1"] = self.rotate(ant_center_x, ant_center_y, conections_sorted_list[3][0], conections_sorted_list[3][1], (overlap_no*pi/2))
        
        
        
        
        outer_box_poly_points = [[-outer_box_width/2, -outer_box_height/2],
        ...
        outer_box_inner_cutout_points = [[-outer_box_inner_cutout_width/2, -outer_box_inner_cutout_height/2],
                                         [-outer_box_inner_cutout_width/2, outer_box_inner_cutout_height/2],
                                         [outer_box_inner_cutout_width/2, outer_box_inner_cutout_height/2],
                                         [outer_box_inner_cutout_width/2, -outer_box_inner_cutout_height/2]]
        ...
        half_conect_poly_points_right = [[half_conect_distacne_from_center, half_conect_taper_inner_height/2],
                                         [half_conect_distacne_from_center + half_conect_taper_diag_width, linewidth/2 + half_conect_taper_extra_height],
                                         [half_conect_distacne_from_center + half_conect_taper_diag_width + half_conect_taper_stright_width, linewidth/2 + half_conect_taper_extra_height],
        ...
        half_conect_poly_points_left = [[-half_conect_distacne_from_center, half_conect_taper_inner_height/2],
                                        [-half_conect_distacne_from_center - half_conect_taper_diag_width, linewidth/2 + half_conect_taper_extra_height],
                                        [-half_conect_distacne_from_center - half_conect_taper_diag_width - half_conect_taper_stright_width, linewidth/2 + half_conect_taper_extra_height],
                                        [-half_conect_distacne_from_center - half_conect_taper_diag_width - half_conect_taper_stright_width, linewidth/2],
                                        [-outer_box_width/2, linewidth/2],
        ...
        half_conect_bridge_rect = gdspy.Rectangle([-half_conect_bridge_rect_width/2, -half_conect_bridge_rect_height/2], [half_conect_bridge_rect_width/2, half_conect_bridge_rect_height/2], **self.Nb_Groundplane)
        half_conect_bridge_rect.rotate(rot, (0,0))
        half_conect_bridge_rect.translate(x, y)
        self.Main.add(half_conect_bridge_rect)
        
        half_conect_bridge_pad_right = gdspy.Rectangle([half_conect_bridge_pad_offset_from_center, -half_conect_bridge_pad_height/2], [half_conect_bridge_pad_offset_from_center + half_conect_bridge_pad_width, half_conect_bridge_pad_height/2], **self.SiN_dep)
        ...
        half_conect_bridge_pad_left = gdspy.Rectangle([half_conect_bridge_pad_offset_from_center, -half_conect_bridge_pad_height/2], [half_conect_bridge_pad_offset_from_center + half_conect_bridge_pad_width, half_conect_bridge_pad_height/2], **self.SiN_dep)
        ...
        full_conect_poly_points = [[full_conect_center_width/2, full_conect_taper_start_from_center],
                                   [linewidth/2 + full_conect_taper_extra_width, full_conect_taper_start_from_center + full_conect_taper_diag_height],
                                   [linewidth/2 + full_conect_taper_extra_width, full_conect_taper_start_from_center + full_conect_taper_diag_height + full_conect_taper_stright_height],
    ...
    def add_Filter_bank_and_get_conection_points(self, x, y, Main_config_file_dict, with_combiner=True, with_crossover=True, only_1_pol=False):
        """
        ...
        half_Hugh_filter_total_arc_angle = np.sum(Hugh_filter_thin_arc_angles) + Hugh_filter_thick_arc_angles[0] + (Hugh_filter_thick_arc_angles[1]/2)
        ...
        ring_overlap_rotations = [self.deg_to_rad(ring_overlap_0_rot),
        ...
        for i in range(4):
            if not only_1_pol or i in [0,2]:
                inner_arc_section = gdspy.Round([x,y], inner_ring_radius+(inner_ring_line_width/2), inner_ring_radius-(inner_ring_line_width/2), initial_angle=inner_ring_overlap_theta, final_angle=inner_ring_overlap_theta+(pi/2)-inner_ring_arc_angle, **self.Nb_Antenna)
                self.Main.add(inner_arc_section)
            
                
            
            if i in [0,1] and with_crossover:
                outer_arc_section1 = gdspy.Round([x,y], outer_ring_radius+(outer_ring_line_width/2), outer_ring_radius-(outer_ring_line_width/2), initial_angle=outer_ring_overlap_theta, final_angle=(i*pi/2)+(pi/4)-outer_combiner_theta, **self.Nb_Antenna)
                outer_arc_section2 = gdspy.Round([x,y], outer_ring_radius+(outer_ring_line_width/2), outer_ring_radius-(outer_ring_line_width/2), initial_angle=(i*pi/2)+(pi/4)+outer_combiner_theta, final_angle=outer_ring_overlap_theta+(pi/2)-outer_ring_arc_angle, **self.Nb_Antenna)
                self.Main.add(outer_arc_section1)
                self.Main.add(outer_arc_section2)
            
            if i in [2,3]:
                if (not only_1_pol) or (with_crossover):
                    outer_arc_section11 = gdspy.Round([x,y], outer_ring_radius+(outer_ring_line_width/2), outer_ring_radius-(outer_ring_line_width/2), initial_angle=outer_ring_overlap_theta, final_angle=(i*pi/2)+(pi/8)-(half_Hugh_filter_total_arc_angle), **self.Nb_Antenna)
                    outer_arc_section12 = gdspy.Round([x,y], outer_ring_radius+(outer_ring_line_width/2), outer_ring_radius-(outer_ring_line_width/2), initial_angle=(i*pi/2)+(pi/8)+(half_Hugh_filter_total_arc_angle), final_angle=(i*pi/2)+(pi/4)-outer_combiner_theta, **self.Nb_Antenna)
                    self.Main.add(outer_arc_section11)
                    self.Main.add(outer_arc_section12)
                
                outer_arc_section21 = gdspy.Round([x,y], outer_ring_radius+(outer_ring_line_width/2), outer_ring_radius-(outer_ring_line_width/2), initial_angle=(i*pi/2)+(pi/4)+outer_combiner_theta, final_angle=(i*pi/2)+(pi/8)+(pi/4)-(half_Hugh_filter_total_arc_angle), **self.Nb_Antenna)
                outer_arc_section22 = gdspy.Round([x,y], outer_ring_radius+(outer_ring_line_width/2), outer_ring_radius-(outer_ring_line_width/2), initial_angle=(i*pi/2)+(pi/8)+(pi/4)+(half_Hugh_filter_total_arc_angle), final_angle=outer_ring_overlap_theta+(pi/2)-outer_ring_arc_angle, **self.Nb_Antenna)
                ...
                for j in range(2):
                    ...
                    if (only_1_pol and not with_crossover) and j==0:
                        ...
                        conection_low_pass_to_hugh_points = [[x + inner_ring_radius*cos(inner_conect_ang), y + inner_ring_radius*sin(inner_conect_ang)],
                        ...
                        conection_low_pass_to_hugh = gdspy.FlexPath(conection_low_pass_to_hugh_points, inner_ring_line_width, corners='circular bend', bend_radius=(outer_ring_radius-inner_ring_radius)/2, **self.Nb_Antenna)
                        ...
                        conection_from_hugh_points = [[x + outer_ring_radius*cos(inner_conect_ang), y + outer_ring_radius*sin(inner_conect_ang)],
                                                      ...
                                                      [x + outer_arc_conection_radius*cos(inner_conect_ang) + extra_length_connect_arc*cos(outer_conect_ang-pi/2),
                                                       y + outer_arc_conection_radius*sin(inner_conect_ang) + extra_length_connect_arc*sin(outer_conect_ang-pi/2)],
                                                      [x + outer_arc_conection_radius*cos(outer_conect_ang), y + outer_arc_conection_radius*sin(outer_conect_ang)]
                                                      ]
                        conection_from_hugh = gdspy.FlexPath(conection_from_hugh_points, inner_ring_line_width, corners='circular bend', bend_radius=(outer_ring_radius-inner_ring_radius)/2, **self.Nb_Antenna)
                        self.Main.add(conection_from_hugh)
                        BR_KID_connect_1_pol = [x + outer_arc_conection_radius*cos(-pi/4), y + outer_arc_conection_radius*sin(-pi/4)]
                        ...
                        inner_to_outer_connect_path_points = [[x + inner_ring_radius*cos(inner_conect_ang), y + inner_ring_radius*sin(inner_conect_ang)],
                        ...
                        inner_to_outer_connect_path = gdspy.FlexPath(inner_to_outer_connect_path_points, inner_ring_line_width, corners='circular bend', bend_radius=(outer_ring_radius-inner_ring_radius), **self.Nb_Antenna)
                        ...
                        conection_high_pass_points = [[x + inner_ring_radius*cos(inner_conect_ang), y + inner_ring_radius*sin(inner_conect_ang)],
                                                      [x + inner_ring_radius*cos(inner_conect_ang) + extra_inner_length_connect_arc*cos(inner_conect_ang+pi/2),
                                                       ...
                                                       [x + outer_ring_radius*cos(outer_conect_ang), y + outer_ring_radius*sin(outer_conect_ang)],
                                                      [x + outer_ring_radius*cos(outer_conect_ang) + extra_outer_length_connect_arc_after, y + outer_ring_radius*sin(outer_conect_ang)]
                                                      ]
                        conection_high_pass = gdspy.FlexPath(conection_high_pass_points, inner_ring_line_width, corners='circular bend', bend_radius=(outer_ring_radius-inner_ring_radius)/2, **self.Nb_Antenna)
                        ...
                        small_connection_left_mid_points = [[x + inner_ring_radius*cos(inner_conect_ang), y + inner_ring_radius*sin(inner_conect_ang)],
                                                            ...
                                                            [x + inner_ring_radius*cos(inner_conect_ang) + extra_length_straight*cos(inner_conect_ang - pi/2),
                                                             y + inner_ring_radius*sin(inner_conect_ang) + extra_length_straight*sin(inner_conect_ang - pi/2) + extra_length_straight]
                                                            ]
                        
                        small_connection_left_mid = gdspy.FlexPath(small_connection_left_mid_points, inner_ring_line_width, corners='circular bend', bend_radius=(outer_ring_radius-inner_ring_radius)/2, **self.Nb_Antenna)
                        ...
                        one_pol_connect_dict = {"TL" : TL_KID_connect_1_pol,
                    ...
                    middle_Hugh_thick_arc = gdspy.Round([x,y], outer_ring_radius+(Hugh_filter_thick_width/2), outer_ring_radius-(Hugh_filter_thick_width/2), initial_angle=(i*pi/2)+mid_Hugh_angle-(Hugh_filter_thick_arc_angles[1]/2), final_angle=(i*pi/2)+mid_Hugh_angle+(Hugh_filter_thick_arc_angles[1]/2), **self.Nb_Antenna)
                    self.Main.add(middle_Hugh_thick_arc)
                    angle_inc+=(Hugh_filter_thick_arc_angles[1]/2)
                    
                    middle_Hugh_thin_arc1 = gdspy.Round([x,y], outer_ring_radius+(Hugh_filter_thin_width/2), outer_ring_radius-(Hugh_filter_thin_width/2), initial_angle=(i*pi/2)+mid_Hugh_angle-angle_inc-Hugh_filter_thin_arc_angles[1], final_angle=(i*pi/2)+mid_Hugh_angle-angle_inc, **self.Nb_Antenna)
                    middle_Hugh_thin_arc2 = gdspy.Round([x,y], outer_ring_radius+(Hugh_filter_thin_width/2), outer_ring_radius-(Hugh_filter_thin_width/2), initial_angle=(i*pi/2)+mid_Hugh_angle+angle_inc, final_angle=(i*pi/2)+mid_Hugh_angle+angle_inc+Hugh_filter_thin_arc_angles[1], **self.Nb_Antenna)
                    self.Main.add(middle_Hugh_thin_arc1)
                    self.Main.add(middle_Hugh_thin_arc2)
                    angle_inc+=Hugh_filter_thin_arc_angles[1]
                    
                    outer_Hugh_thick_arc1 = gdspy.Round([x,y], outer_ring_radius+(Hugh_filter_thick_width/2), outer_ring_radius-(Hugh_filter_thick_width/2), initial_angle=(i*pi/2)+mid_Hugh_angle-angle_inc-Hugh_filter_thick_arc_angles[0], final_angle=(i*pi/2)+mid_Hugh_angle-angle_inc, **self.Nb_Antenna)
                    outer_Hugh_thick_arc2 = gdspy.Round([x,y], outer_ring_radius+(Hugh_filter_thick_width/2), outer_ring_radius-(Hugh_filter_thick_width/2), initial_angle=(i*pi/2)+mid_Hugh_angle+angle_inc, final_angle=(i*pi/2)+mid_Hugh_angle+angle_inc+Hugh_filter_thick_arc_angles[0], **self.Nb_Antenna)
                    self.Main.add(outer_Hugh_thick_arc1)
                    self.Main.add(outer_Hugh_thick_arc2)
                    angle_inc+=(Hugh_filter_thick_arc_angles[0])
                    
                    outer_Hugh_thin_arc1 = gdspy.Round([x,y], outer_ring_radius+(Hugh_filter_thin_width/2), outer_ring_radius-(Hugh_filter_thin_width/2), initial_angle=(i*pi/2)+mid_Hugh_angle-angle_inc-Hugh_filter_thin_arc_angles[0], final_angle=(i*pi/2)+mid_Hugh_angle-angle_inc, **self.Nb_Antenna)
                    outer_Hugh_thin_arc2 = gdspy.Round([x,y], outer_ring_radius+(Hugh_filter_thin_width/2), outer_ring_radius-(Hugh_filter_thin_width/2), initial_angle=(i*pi/2)+mid_Hugh_angle+angle_inc, final_angle=(i*pi/2)+mid_Hugh_angle+angle_inc+Hugh_filter_thin_arc_angles[0], **self.Nb_Antenna)
            ...
            if i==0:
                self.add_Lo_pass_filters(x, y, inner_ring_line_width, inner_ring_radius, (i*pi/2)+pi/4, "clockwise", Main_config_file_dict)
                self.add_Hi_pass_filters(x, y, inner_ring_line_width, inner_ring_radius, (i*pi/2)+pi/4, "anti-clockwise", Main_config_file_dict)
            if i==1 and not only_1_pol:
                self.add_Hi_pass_filters(x, y, inner_ring_line_width, inner_ring_radius, (i*pi/2)+pi/4, "clockwise", Main_config_file_dict)
                self.add_Lo_pass_filters(x, y, inner_ring_line_width, inner_ring_radius, (i*pi/2)+pi/4, "anti-clockwise", Main_config_file_dict)
            if i==2:
                self.add_Hi_pass_filters(x, y, inner_ring_line_width, inner_ring_radius, (i*pi/2)+pi/4, "clockwise", Main_config_file_dict)
                self.add_Lo_pass_filters(x, y, inner_ring_line_width, inner_ring_radius, (i*pi/2)+pi/4, "anti-clockwise", Main_config_file_dict)
            ...
            if with_combiner:
                ...
                if i in [0,1]:
                    conection_point = self.add_combiner_section_and_get_conect_point(combiner_xpos,combiner_ypos, (pi/4 + i*pi/2), outer_ring_conector_gap, outer_ring_line_width, Main_config_file_dict, combiner_type="150GHZ", mirror_combiner=mirror_combiner)
                else:
                    conection_point = self.add_combiner_section_and_get_conect_point(combiner_xpos,combiner_ypos, (pi/4 + i*pi/2), outer_ring_conector_gap, outer_ring_line_width, Main_config_file_dict, combiner_type="90GHZ", mirror_combiner=mirror_combiner)
            else:
                conection_point = [x + outer_ring_radius*cos((i*pi/2)+(pi/4)-outer_combiner_theta), y + outer_ring_radius*sin((i*pi/2)+(pi/4)-outer_combiner_theta)]
            ...
            if with_crossover:
                ...
                conections_dict = self.add_filter_bank_ring_overlap_and_get_conections(ring_overlap_xpos, ring_overlap_ypos, x, y, ring_overlap_number, ring_overlap_rotations[i], Main_config_file_dict)
                
                #inner conections to ring overlap
                inner_0_conection_points = [conections_dict["inner_conect_0"],
                                           [conections_dict["inner_conect_0"][0] + inner_straight_extend_distance*cos(-3*pi/4 + i*pi/2), conections_dict["inner_conect_0"][1] + inner_straight_extend_distance*sin(-3*pi/4 + i*pi/2)],
                                           [x+(inner_ring_radius*cos((i*pi/2) - inner_ring_arc_angle/2 + inner_straight_extend_distance/inner_ring_radius)), y+(inner_ring_radius*sin((i*pi/2) - inner_ring_arc_angle/2 + inner_straight_extend_distance/inner_ring_radius))],
                                           [x+(inner_ring_radius*cos((i*pi/2) - inner_ring_arc_angle/2)), y+(inner_ring_radius*sin((i*pi/2) - inner_ring_arc_angle/2))]]
                
                inner_0_path = gdspy.FlexPath(inner_0_conection_points, inner_ring_line_width, corners='circular bend', bend_radius=inner_bend_radius, **self.Nb_Antenna)
                self.Main.add(inner_0_path)
                
                inner_1_conection_points = [conections_dict["inner_conect_1"],
                                           [conections_dict["inner_conect_1"][0] + inner_straight_extend_distance*cos(3*pi/4 + i*pi/2), conections_dict["inner_conect_1"][1] + inner_straight_extend_distance*sin(3*pi/4 + i*pi/2)],
                                           [x+(inner_ring_radius*cos((i*pi/2) + inner_ring_arc_angle/2 - inner_straight_extend_distance/inner_ring_radius)), y+(inner_ring_radius*sin((i*pi/2) + inner_ring_arc_angle/2 - inner_straight_extend_distance/inner_ring_radius))],
                                           [x+(inner_ring_radius*cos((i*pi/2) + inner_ring_arc_angle/2)), y+(inner_ring_radius*sin((i*pi/2) + inner_ring_arc_angle/2))]]
                
                inner_1_path = gdspy.FlexPath(inner_1_conection_points, inner_ring_line_width, corners='circular bend', bend_radius=inner_bend_radius, **self.Nb_Antenna)
                self.Main.add(inner_1_path)
                
                #outer conections to ring overlap
                outer_0_conection_points = [conections_dict["outer_conect_0"],
                                           [conections_dict["outer_conect_0"][0] + outer_straight_extend_distance*cos((-pi/4 + i*pi/2)), conections_dict["outer_conect_0"][1] + outer_straight_extend_distance*sin((-pi/4 + i*pi/2 ))],
                                           [x+(outer_ring_radius*cos((i*pi/2) - outer_ring_arc_angle/2 + outer_straight_extend_distance/outer_ring_radius)), y+(outer_ring_radius*sin((i*pi/2) - outer_ring_arc_angle/2 + outer_straight_extend_distance/outer_ring_radius))],
                                           [x+(outer_ring_radius*cos((i*pi/2) - outer_ring_arc_angle/2)), y+(outer_ring_radius*sin((i*pi/2) - outer_ring_arc_angle/2))]]
                
                outer_0_path = gdspy.FlexPath(outer_0_conection_points, inner_ring_line_width, corners='circular bend', bend_radius=outer_bend_radius, **self.Nb_Antenna)
                self.Main.add(outer_0_path)
                
                outer_1_conection_points = [conections_dict["outer_conect_1"],
                                           [conections_dict["outer_conect_1"][0] + outer_straight_extend_distance*cos((pi/4 + i*pi/2)), conections_dict["outer_conect_1"][1] + outer_straight_extend_distance*sin((pi/4 + i*pi/2))],
                                           [x+(outer_ring_radius*cos((i*pi/2) + outer_ring_arc_angle/2 - outer_straight_extend_distance/outer_ring_radius)), y+(outer_ring_radius*sin((i*pi/2) + outer_ring_arc_angle/2 - outer_straight_extend_distance/outer_ring_radius))],
                                           [x+(outer_ring_radius*cos((i*pi/2) + outer_ring_arc_angle/2)), y+(outer_ring_radius*sin((i*pi/2) + outer_ring_arc_angle/2))]]
                
                right_path = gdspy.FlexPath(outer_1_conection_points, inner_ring_line_width, corners='circular bend', bend_radius=outer_bend_radius, **self.Nb_Antenna)
    ...
    def connect_filter_bank_to_KIDs(self, x, y, absolute_filter_bank_points, relative_kid_positions, only_1_pol_no_comb=False):
        ...
        KID_TR = (relative_kid_positions[1][0] + x, relative_kid_positions[1][1] + y)# these are the coords of the base of each of the KIDS, TopLeft, TopRight, BotLeft, BotRight
        ...
        for i in range(4):
            ...
            points_taper_to_FILT = [[filt_coord[0] + filter_extend_out_distance*cos(filt_rot), filt_coord[1] + filter_extend_out_distance*sin(filt_rot)],
                                    filt_coord
            ...
            angle = np.arctan2((points_taper_to_FILT[0][1] - points_KID_to_taper[-1][1]), (points_taper_to_FILT[0][0] - points_KID_to_taper[-1][0]))
            
            taper_kid_side_to_filt_side = [[points_KID_to_taper[-1][0] + 2*kid_side_bend_radius*cos(angle), points_KID_to_taper[-1][1] + 2*kid_side_bend_radius*sin(angle)],
                                           [points_taper_to_FILT[0][0] + 2*filt_side_bend_radius*cos(angle+pi), points_taper_to_FILT[0][1] + 2*filt_side_bend_radius*sin(angle+pi)]
                                           ] # this calculates the middle ends points of the taper
            
            taper_points = [[taper_kid_side_to_filt_side[0][0] + (KID_conection_linewidth/2)*cos(angle-pi/2), taper_kid_side_to_filt_side[0][1] + (KID_conection_linewidth/2)*sin(angle-pi/2)],
                            [taper_kid_side_to_filt_side[0][0] - (KID_conection_linewidth/2)*cos(angle-pi/2), taper_kid_side_to_filt_side[0][1] - (KID_conection_linewidth/2)*sin(angle-pi/2)],
                            [taper_kid_side_to_filt_side[1][0] - (filter_conection_linewidth/2)*cos(angle-pi/2), taper_kid_side_to_filt_side[1][1] - (filter_conection_linewidth/2)*sin(angle-pi/2)],
                            [taper_kid_side_to_filt_side[1][0] + (filter_conection_linewidth/2)*cos(angle-pi/2), taper_kid_side_to_filt_side[1][1] + (filter_conection_linewidth/2)*sin(angle-pi/2)]
            ...
            points_KID_to_taper.append(taper_kid_side_to_filt_side[0]) # appends the start of the taper to the flex path points from the kid so the taper conects the two paths
            
            points_taper_to_FILT.insert(0, taper_kid_side_to_filt_side[1]) # inserts the end of the taper at the start of the flex path points for the filter side to again ensure it conects up
            
            # creates the flex paths for each side
            KID_to_taper = gdspy.FlexPath(points_KID_to_taper, KID_conection_linewidth, corners="circular bend", bend_radius=kid_side_bend_radius, gdsii_path=True, **self.Nb_Antenna)
            taper_to_FILT = gdspy.FlexPath(points_taper_to_FILT, filter_conection_linewidth, corners="circular bend", bend_radius=filt_side_bend_radius, gdsii_path=True, **self.Nb_Antenna)
    ...
    def connect_ants_to_Filter_Bank(self, x, y, relative_antena_conect_positions, antena_rot, Main_config_file_dict, terminate_ants=[]):
        """
        ...
        CPW_tans_lens = [config["CPW_tans_len1"], config["CPW_tans_len2"], config["CPW_tans_len3"], config["CPW_tans_len4"], config["CPW_tans_len5"]]
        ...
        trans2 = gdspy.Rectangle([np.sum(gaps[0:1])+np.sum(CPW_tans_lens[0:1])+gaps[1], -CPW_width/2], [np.sum(gaps[0:1])+np.sum(CPW_tans_lens[0:1])+gaps[1]+CPW_tans_lens[1], CPW_width/2])
        trans3 = gdspy.Rectangle([np.sum(gaps[0:2])+np.sum(CPW_tans_lens[0:2])+gaps[2], -CPW_width/2], [np.sum(gaps[0:2])+np.sum(CPW_tans_lens[0:2])+gaps[2]+CPW_tans_lens[2], CPW_width/2])
        trans4 = gdspy.Rectangle([np.sum(gaps[0:3])+np.sum(CPW_tans_lens[0:3])+gaps[3], -CPW_width/2], [np.sum(gaps[0:3])+np.sum(CPW_tans_lens[0:3])+gaps[3]+CPW_tans_lens[3], CPW_width/2])
        trans5 = gdspy.Rectangle([np.sum(gaps[0:4])+np.sum(CPW_tans_lens[0:4])+gaps[4], -CPW_width/2], [np.sum(gaps[0:4])+np.sum(CPW_tans_lens[0:4])+gaps[4]+CPW_tans_lens[4], CPW_width/2])
        ...
        ANT_L = [relative_antena_conect_positions[2][0] + x, relative_antena_conect_positions[2][1] + y] # these are the coords of the end of the antennas , Left, Top, Right, Bot
        ...
        FILT_TR = [x + inner_ring_radius*cos(pi/4 + 0*(pi/2)), y + inner_ring_radius*sin(pi/4 + 0*(pi/2))] # these are the coords of the filter bank conect positions, TopRight, TopLeft, BotLeft, BotRight
        ...
        extend_out_distance = config["extend_out_distance"] # the distance ove which the feedline is straight coming out of the antenna and Filter Bank
        ...
        for i in range(4):
            ...
            extended_ANT = [ANT[0] + extend_out_distance*cos(self.deg_to_rad(antena_rot + (i*90))),
                            ANT[1] + extend_out_distance*sin(self.deg_to_rad(antena_rot + (i*90)))]
            
            extended_FILT = [FILT[0] + extend_out_distance*cos(3*pi/4 + i*(pi/2)),
            ...
            tenth_pnt_straight = [extended_ANT[0] + (middle_straight_dx/10),
                                  extended_ANT[1] + (middle_straight_dy/10)]
            
            trans_end = [tenth_pnt_straight[0] + tot_trans_length*cos(middle_straight_angle),
                         tenth_pnt_straight[1] + tot_trans_length*sin(middle_straight_angle)]
            
            ANT_to_TRANS_path = gdspy.FlexPath([ANT, extended_ANT, tenth_pnt_straight], ant_line_width, corners="circular bend", bend_radius=flex_path_bend_radius, **self.Nb_Antenna)
            self.Main.add(ANT_to_TRANS_path)
            
            cutout_around_ANT_to_TANS_polys = self.get_polys_from_flexpath(gdspy.FlexPath([ANT, extended_ANT, tenth_pnt_straight] , CPW_width, corners="circular bend", bend_radius=flex_path_bend_radius, gdsii_path=True))
            ...
            TRANS_to_FILT_path = gdspy.FlexPath([trans_end, extended_FILT, FILT], microstrip_lw, corners="circular bend", bend_radius=flex_path_bend_radius, **self.Nb_Antenna)
            self.Main.add(TRANS_to_FILT_path)
            
            TRANS_cutouts = gdspy.CellReference(transition, tenth_pnt_straight, rotation=(self.rad_to_deg(middle_straight_angle)) ) # adds the transition geometry between CPW and microstrip antenna a fith of the way down the middle straight
            self.ground_plane_cutouts.add(TRANS_cutouts)
            
            points = self.rotate_and_move_points_list(cpw_transition_poly_points, middle_straight_angle, tenth_pnt_straight[0], tenth_pnt_straight[1])
        ...
        for i in add_termination_meander_indexes:
            ...
            bend_center_xy = [x + (inner_ring_radius + (meander_bend_height/2))*cos(start_ang - arc_theta_close), y + (inner_ring_radius + (meander_bend_height/2))*sin(start_ang - arc_theta_close)]
            
            arc_close = gdspy.Round([x,y], radius=(inner_ring_radius + (meander_width/2)), inner_radius=(inner_ring_radius - (meander_width/2)), initial_angle=(start_ang), final_angle=(start_ang - arc_theta_close), **self.Aluminium)
            
            arc_bend = gdspy.Round(bend_center_xy, radius=((meander_bend_height/2) + (meander_width/2)), inner_radius=((meander_bend_height/2) - (meander_width/2)), initial_angle=(start_ang - arc_theta_close-pi), final_angle=(start_ang - arc_theta_close), **self.Aluminium)
            
            arc_far = gdspy.Round([x,y],  radius=(inner_ring_radius + meander_bend_height + (meander_width/2)), inner_radius=(inner_ring_radius + meander_bend_height - (meander_width/2)), initial_angle=(start_ang - arc_theta_close +arc_theta_far ), final_angle=(start_ang - arc_theta_close), **self.Aluminium)
            
            overlap_ang = (overlap_pad_width/2)/inner_ring_radius
            conect_overlap = gdspy.Round([x,y], radius=(inner_ring_radius + (overlap_pad_height/2)), inner_radius=(inner_ring_radius - (overlap_pad_height/2)), initial_angle=(start_ang - overlap_ang), final_angle=(start_ang + overlap_ang), **self.Aluminium)
    ...
    def connect_ants_to_KIDs(self, x, y, relative_antena_conect_positions, relative_kid_positions, antena_rot, Main_config_file_dict):
        ...
        for i in range(4):
            ...
            extended_KID = [KID[0] + extend_out_distance*cos(KIDS_ROTS[i]),
                            KID[1] + extend_out_distance*sin(KIDS_ROTS[i])]
            ...
            TRANS_to_KID_path = gdspy.FlexPath([trans_end, extended_KID, KID], microstrip_lw, corners="circular bend", bend_radius=flex_path_bend_radius, **self.Nb_Antenna)
    ...
    def add_feedline_and_dielectric(self, feed_points, Main_config_file_dict, center_material="Nb", add_bridges=True, points_to_avoid_bridging=None):
        """
        ...
        if center_material=="Nb":
            center_path = gdspy.FlexPath(feed_points, feedline_width, corners="circular bend", bend_radius=bend_radius, gdsii_path=True, **self.Nb_Antenna)
            self.Main.add(center_path)
            
        if center_material=="Al":
            center_path = gdspy.FlexPath(feed_points, feedline_width, corners="circular bend", bend_radius=bend_radius, gdsii_path=True, **self.Aluminium)
            self.Main.add(center_path)
        
        if center_material=="Both":
            center_path1 = gdspy.FlexPath(feed_points, feedline_width, corners="circular bend", bend_radius=bend_radius, gdsii_path=True, **self.Aluminium)
            center_path2 = gdspy.FlexPath(feed_points, feedline_width, corners="circular bend", bend_radius=bend_radius, gdsii_path=True, **self.Nb_Antenna)
            self.Main.add(center_path1)
            self.Main.add(center_path2)
            
        
        ground_plane_cuttout_outer_path = gdspy.FlexPath(feed_points, cutout_around_feedline_width, corners="circular bend", bend_radius=bend_radius, gdsii_path=True) # makes the outer of the cpw flex path
        poly_points_flexpath = self.get_polys_from_flexpath(ground_plane_cuttout_outer_path) # gets the polygon points of the outer cpw flex path via function and
        ...
        for i in range(len(poly_points_flexpath)):
            ...
            self.ground_plane_cutouts.add([port_to_fdln_outer_path_polygon]) # makes a polygon with that so it can be cutout of the ground plane (flex path cannot be added directly as boolean operation cant cut that out)
        
        
        
        dielectric_under_feedline_path = gdspy.FlexPath(feed_points, dielectric_under_feedline_width, corners="circular bend", bend_radius=bend_radius, gdsii_path=True, **self.SiN_dep)
        ...
        for i in range(len(lengths_of_path_sections)):
            lengths_of_path_sections[i] = ( (rounded_feedline_points[i][0] - rounded_feedline_points[i+1][0])**2 + (rounded_feedline_points[i][1] - rounded_feedline_points[i+1][1])**2 )**0.5
            angles_of_path_sections[i] = np.arctan2( (rounded_feedline_points[i+1][1] - rounded_feedline_points[i][1]), (rounded_feedline_points[i+1][0] - rounded_feedline_points[i][0]) )
        ...
        for i in range(no_of_bridges):        
            ...
            x_bridge_pos = rounded_feedline_points[arg][0] + (rounded_feedline_points[arg+1][0] - rounded_feedline_points[arg][0])*length_perc_from_prev_point
            y_bridge_pos = rounded_feedline_points[arg][1] + (rounded_feedline_points[arg+1][1] - rounded_feedline_points[arg][1])*length_perc_from_prev_point
            
            if points_to_avoid_bridging == None:
                bridge = gdspy.Rectangle([x_bridge_pos-(bridge_width/2), y_bridge_pos-(bridge_height/2)], [x_bridge_pos+(bridge_width/2), y_bridge_pos+(bridge_height/2)], **self.Nb_Groundplane)
    ...
    def add_arbitrary_CPW(self, feed_points, bend_radius, feedline_width, cutout_around_feedline_width, dielectric_under_feedline_width, center_material="Nb", add_bridges=False, cutout_dilectric_around_end_distance=0):
        """
        ...
        dielectric_under_feedline_width : float, int
            The width of the dielectric that covers the center line and the
            cutout around that.
        ...
        if cutout_dilectric_around_end_distance != 0:
            ...
            path_points = [[feed_points[-1][0] + cutout_dilectric_around_end_distance*cos(angle_of_last_section),
                            feed_points[-1][1] + cutout_dilectric_around_end_distance*sin(angle_of_last_section)],
                           feed_points[-1]]
            
            dielectric_cutout_at_end_of_path = gdspy.FlexPath(path_points, dielectric_under_feedline_width*1.5, corners="circular bend", bend_radius=bend_radius, gdsii_path=True, **self.SiN_dep)
    ...
    def add_port_and_get_connection_point(self, x, y, rotation, Main_config_file_dict, center_material="Nb", add_extra_squares=True, dielectric_cutout_in_port=True):
        """
        ...
        port_center_points = [[x,                                    y + (outer_feedline_width/2)],
        ...
        port_cutout_around_points = [[x,                                    y + (outer_cutout_around_feedline_width/2)],
        ...
        port_dielectric_points = [[x,                                    y + (outer_dielectric_under_feedline_width/2)],
        ...
        if dielectric_cutout_in_port:
            port_dielectric_cutout_points = [[x,                                    y + (dielectric_cutout_in_port_width/2)],
        ...
        if add_extra_squares:
            ...
            for i in range(no_of_squares):
                ...
                square_top = gdspy.Rectangle([x + square_offset_from_edge, y + offset_from_center],
                ...
                square_bot = gdspy.Rectangle([x + square_offset_from_edge, y - offset_from_center],
                                             [x + square_offset_from_edge + square_side_len, y - offset_from_center - square_side_len],
                                             **self.Nb_Antenna)
    ...
    def get_feedline_pass_through_points(self, x, y, relative_kid_positions, Main_config_file_dict):
        ...
        vert_feedline_offset_from_kid_position = ( (res_config["meander_bot_width"]/2)
        ...
        horizontal_feedline_offset_from_kid_position = ( res_config["meander_lw"]
        ...
        feed_pass_points.append([x + relative_kid_positions[0][0] - horizontal_feedline_offset_from_kid_position,
                                 (y + relative_kid_positions[0][1] + vert_feedline_offset_from_kid_position + 36/2)])#TL
        
        feed_pass_points.append([x + relative_kid_positions[1][0] + horizontal_feedline_offset_from_kid_position,
                                 (y + relative_kid_positions[1][1] + vert_feedline_offset_from_kid_position + 36/2)])#TR
        
        feed_pass_points.append([x + relative_kid_positions[2][0] - horizontal_feedline_offset_from_kid_position,
                                 (y + relative_kid_positions[2][1] - vert_feedline_offset_from_kid_position - 36/2)])#BL
        
        feed_pass_points.append([x + relative_kid_positions[3][0] + horizontal_feedline_offset_from_kid_position,
                                 (y + relative_kid_positions[3][1] - vert_feedline_offset_from_kid_position - 36/2)])#BR
    ...
    def get_total_height_of_resonator(self, Main_config_file_dict):
        ...
        KID_height = ( res_config["meander_lw"]
                       + res_config["meander_left_height_1"]
                       + res_config["meander_left_height_2"]
                       + res_config["meander_left_height_3"]
                       + res_config["frame_left_height"]
    ...
    def get_width_height_of_resonator_IDC_section(self, Main_config_file_dict):
        ...
        KID_height = ( res_config["cutout_bot_offset"]
                       + res_config["frame_left_height"]
                       + res_config["coupler_frame_left_height"]
                       + res_config["coupler_gap"]
                       + res_config["coupler_lw"]
        ...
        left_side_width = ( (res_config["meander_bot_width"]/2)
                            - (res_config["meander_lw"]/2)
                            + (res_config["meander_left_width_1"] - res_config["meander_left_width_2"])
                            + res_config["frame_bot_left_width"]
        ...
        right_side_width = ( (res_config["meander_bot_width"]/2)
                             - (res_config["meander_lw"]/2)
    ...
    def get_feedline_center_to_meander_base_distance(self, Main_config_file_dict):
        ...
        feedline_to_meander_base_distance = ( (res_config["meander_bot_width"]/2)
                                              - (res_config["meander_lw"]/2)
                                              + (res_config["meander_left_width_1"] - res_config["meander_left_width_2"])
                                              + res_config["frame_bot_left_width"]
                                              - (res_config["frame_left_lw"]/2)
    ...
    def get_relative_kid_positions(self, Main_config_file_dict):
        ...
        horizontal_offset_from_center_of_antenna = (Main_config_file_dict["general"]["horizontal_pitch"]/2) - tot_kid_height - (ground_gap_between_resonators/2)
        ...
        rel_kid_positions = ( [-horizontal_offset_from_center_of_antenna, +vertical_offset_from_center_of_antenna],
    ...
    def get_relative_antena_conect_positions(self, rotation, Main_config_file_dict):
        ...
        for angle in angles:
            rel_ant_conect_positions.append([ant_connect_offset_from_center*cos(angle),
                                             ant_connect_offset_from_center*sin(angle)
                                             ])
    ...
    def add_sma_connector_and_launcher_and_get_connection(self, x, y, rot, Main_config_file_dict, bend="none"):
        ...
        big_box = gdspy.Rectangle([x - (sma_square_width/2) - sma_square_offset_left, y - (sma_square_height/2) - sma_square_offset_bot],
        ...
        sma_box = gdspy.Rectangle([x - (sma_square_width/2), y - (sma_square_height/2)],
                                  [x + (sma_square_width/2), y + (sma_square_height/2)],
                                  layer=401)
        ...
        cutout_box = gdspy.Rectangle([x + (sma_square_width/2) + sma_square_offset_right - taper_length, y - (dielectirc_cutout_height/2)],
        ...
        if (bend != "left") and (bend != "right"):
            ...
            central_section_poly_points = [[x - (sma_square_width/2) - sma_square_offset_left, y + (central_linewidth/2)],
            ...
            grnd_cutout_poly_points = [[x - (sma_square_width/2) - sma_square_offset_left, y + (central_linewidth/2) + gap],
            ...
            dielectric_poly_points = [[x - (sma_square_width/2) - sma_square_offset_left, y + (central_linewidth/2) + gap + dielectric_overlap_distance],
                                      [x + (sma_square_width/2) + sma_square_offset_right - taper_length, y + (central_linewidth/2) + gap + dielectric_overlap_distance],
        ...
        taper_length = (sma_square_height/2) + sma_square_offset_top - (central_linewidth/2) - gap - dielectric_overlap_distance - extra_bend_center
        ...
        dielectric_poly_points = [[x - (sma_square_width/2) - sma_square_offset_left, y + (central_linewidth/2) + gap + dielectric_overlap_distance],
                                  [x + (sma_square_width/2) + sma_square_offset_right - taper_length - bend_offset_from_end, y + (central_linewidth/2) + gap + dielectric_overlap_distance],
                                  [x + (sma_square_width/2) + sma_square_offset_right - taper_length - bend_offset_from_end, y - (central_linewidth/2) - gap - dielectric_overlap_distance],
                                  [x - (sma_square_width/2) - sma_square_offset_left, y - (central_linewidth/2) - gap - dielectric_overlap_distance]
                                  ]
        ...
        bend_center_xy = [x + (sma_square_width/2) + sma_square_offset_right - taper_length - bend_offset_from_end,
        ...
        dielectric_section_outer_bend_radius = dielectric_section_inner_bend_radius + (2*dielectric_overlap_distance) + (2*gap) + central_linewidth
        
        dielectric_section_bend = gdspy.Round(bend_center_xy, dielectric_section_outer_bend_radius, inner_radius=dielectric_section_inner_bend_radius, initial_angle=0, final_angle=final_bend_angle, **self.SiN_dep)
        ...
        grnd_cutout_bend = gdspy.Round(bend_center_xy, grnd_cutout_outer_bend_radius, inner_radius=grnd_cutout_inner_bend_radius, initial_angle=0, final_angle=final_bend_angle, **self.Nb_Groundplane)
        ...
        central_section_bend = gdspy.Round(bend_center_xy, central_section_outer_bend_radius, inner_radius=central_section_inner_bend_radius, initial_angle=0, final_angle=final_bend_angle, **self.Nb_Antenna)
        ...
        central_taper_poly_points = [[end_of_round_x - (central_linewidth/2), end_of_round_y],
                                     [end_of_round_x - (feedline_width/2), end_of_round_y + sign*taper_length],
                                     [end_of_round_x + (feedline_width/2), end_of_round_y + sign*taper_length],
                                     [end_of_round_x + (central_linewidth/2), end_of_round_y]
                                     ]
        
        grnd_cutout_taper_poly_points = [[end_of_round_x - (central_linewidth/2) - gap, end_of_round_y],
        ...
        dielectric_taper_poly_points = [[end_of_round_x - (central_linewidth/2) - gap - dielectric_overlap_distance, end_of_round_y],
    ...
    def get_sma_connector_and_laucher_bounding_box(self, x, y, rot, Main_config_file_dict):
        ...
        bounding_box_points = [[x - (sma_square_width/2) - sma_square_offset_left, y + (sma_square_height/2) + sma_square_offset_top],
                               [x + (sma_square_width/2) + sma_square_offset_right, y + (sma_square_height/2) + sma_square_offset_top],
    ...
    def remove_exclusions_from_hex_pack(self, hex_pack, exclusions_list, Main_config_file_dict):
        ...
        for hex_pack_point_xy in hex_pack:
            ...
            polygon_surrounding_kid_block = shapely_geom.Polygon([[x - (kid_block_horizontal_size/2), y + (kid_block_vertical_size/2)],
    ...
    def add_center_pin_and_get_bounding_points(self, center_pin_xy, center_pin_radius):
        ...
        bounding_points = []
        for i in range(200):
            bounding_points.append([center_pin_xy[0] + center_pin_radius*cos(i*center_pin_radius),
                                    center_pin_xy[1]] + center_pin_radius*sin(i*center_pin_radius))
    ...
    def add_slotted_pin_and_get_bounding_points(self, slotted_pin_center, slotted_pin_length, slotted_pin_radius, center_pin_xy):
        ...
        end_points_slot = [[slotted_pin_center[0] + slotted_pin_length*cos(slotted_pin_angle), slotted_pin_center[1] + slotted_pin_length*sin(slotted_pin_angle)],
                           [slotted_pin_center[0] + slotted_pin_length*cos(slotted_pin_angle+pi), slotted_pin_center[1] + slotted_pin_length*sin(slotted_pin_angle+pi)]
                           ]
        ...
        for i in range(100):
            bounding_points.append([end_points_slot[0][0] + slotted_pin_radius*cos( loop_angle ),
                                    end_points_slot[0][1] + slotted_pin_radius*sin( loop_angle )
                                    ])
            loop_angle-=ang_diff
        
        for i in range(100, 200):
            bounding_points.append([end_points_slot[1][0] + slotted_pin_radius*cos( loop_angle  ),
    ...
    def add_bottom_choke_features(self, x, y, rel_kid_positions, Main_config_file_dict):
        ...
        y_sign = [+1, +1, -1, -1]
        for i in range(4):
            pad_xy = [x + x_sign[i]*pad_x_offset_from_center,
                      y + y_sign[i]*pad_y_offset_from_center
                      ]
        ...
        for i in range(4):
            ...
            center_of_IDC_cutout_xy = [x + rel_kid_positions[i][0] + x_sign[i]*horizontal_offset_from_rel_kid_position_to_center_of_IDC_cutout,
                                    y + rel_kid_positions[i][1]
                                    ]
            
            IDC_cutout = gdspy.Rectangle([center_of_IDC_cutout_xy[0] - (IDC_cutout_width_height[1]/2) - IDC_cutout_offset_left,
    ...
    def add_hexagonal_tabbed_dicing_line(self, hexagon_points, hex_rad, chip_center_xy, all_sma_conectors_xy_rot):
        ...
        for sma_xy_rot in all_sma_conectors_xy_rot:
            ...
            main_dice_line_center = [x + dice_x_offset_from_center*cos(rot),
                                     y + dice_x_offset_from_center*sin(rot)]

            main_dice_line_start_end = [[main_dice_line_center[0] + dice_y_offset_from_center*cos(rot + pi/2),
            ...
            test_box = gdspy.Rectangle([x + dice_x_offset_from_center, y + dice_y_offset_from_center],
                                       [x - 4*dice_x_offset_from_center, y - dice_y_offset_from_center], layer=5000)
        ...
        hex_line_test = gdspy.FlexPath(new_outline_points, diceline_linewidth, offset=-diceline_linewidth/2, ends="flush" ,**self.Tab_dicing_line)
        self.Main.add(hex_line_test)
        
        
        left_end = [new_outline_points[-1][0] + (diceline_linewidth/2)*cos(-5*pi/6),
                    new_outline_points[-1][1] + (diceline_linewidth/2)*sin(-5*pi/6)]
        
        right_end = [new_outline_points[0][0] + (diceline_linewidth/2)*cos(-5*pi/6),
        ...
        center_point = [(left_end[0] + right_end[0])/2,
                        (left_end[1] + right_end[1])/2]
        
        side_angle = -pi/3
        
        middle_tab = gdspy.FlexPath([[center_point[0] - diceline_tab_length/2, center_point[1]],
                                     [center_point[0] + diceline_tab_length/2, center_point[1]]], diceline_linewidth, ends="round", **self.Tab_dicing_line)
        ...
        for i in range( int(length_between_ends / (diceline_tab_length + diceline_tab_gap + diceline_linewidth)) ):
            ...
            if (diceline_tab_length/2 + i*(diceline_tab_length + diceline_tab_gap)) > length_between_ends/2 :
                ...
                tab_right = gdspy.FlexPath([[center_point[0] - diceline_tab_length/2 + i*(diceline_tab_length + diceline_tab_gap + diceline_linewidth), center_point[1]],
                                            [center_point[0] + length_between_ends/2 + diceline_end_extend_length, center_point[1]]],
                                           diceline_linewidth, ends="flush", **self.Tab_dicing_line)
                ...
                cap_right = gdspy.Round([center_point[0] - diceline_tab_length/2 + i*(diceline_tab_length + diceline_tab_gap + diceline_linewidth), center_point[1]],
                                        diceline_linewidth/2, initial_angle=pi/2, final_angle=3*pi/2, **self.Tab_dicing_line)
                cap_right.rotate(side_angle, center_point)
                self.Main.add(cap_right)
                
                tab_left = gdspy.FlexPath([[center_point[0] - length_between_ends/2 - diceline_end_extend_length, center_point[1]],
                                           [center_point[0] + diceline_tab_length/2 - i*(diceline_tab_length + diceline_tab_gap + diceline_linewidth), center_point[1]]],
                                          diceline_linewidth, ends="flush", **self.Tab_dicing_line)
                ...
                cap_left = gdspy.Round([center_point[0] + diceline_tab_length/2 - i*(diceline_tab_length + diceline_tab_gap + diceline_linewidth), center_point[1]],
                                        diceline_linewidth/2, initial_angle=pi/2, final_angle=-pi/2, **self.Tab_dicing_line)
            ...
            tab_right = gdspy.FlexPath([[center_point[0] - diceline_tab_length/2 + i*(diceline_tab_length + diceline_tab_gap + diceline_linewidth), center_point[1]],
                                  [center_point[0] + diceline_tab_length/2 + i*(diceline_tab_length + diceline_tab_gap + diceline_linewidth), center_point[1]]],
                                       diceline_linewidth, ends="round", **self.Tab_dicing_line)
            tab_right.rotate(side_angle, center_point)
            self.Main.add(tab_right)
            
            tab_left = gdspy.FlexPath([[center_point[0] - diceline_tab_length/2 - i*(diceline_tab_length + diceline_tab_gap + diceline_linewidth), center_point[1]],
                                  [center_point[0] + diceline_tab_length/2 - i*(diceline_tab_length + diceline_tab_gap + diceline_linewidth), center_point[1]]],
                                      diceline_linewidth, ends="round", **self.Tab_dicing_line)
            tab_left.rotate(side_angle, center_point)
            self.Main.add(tab_left)
    ...
    def do_boolean_operations(self):
        ...
        '''
        print("NOW DOING THE BOOL")
        # with alive_bar(title="Doing Boolean", manual=True, calibrate=50, stats=True, force_tty=True, length=20, max_cols=70) as bar:
            # bar.text("Nb Ground Plane")
        groundplane = gdspy.boolean(self.ground_plane_positives.get_polygons([30,0]), self.ground_plane_cutouts, "not", precision=0.01, **self.Nb_Groundplane)
        self.Main.add(groundplane) # then adds this groundplane geometry to the main cell 
        print("Nb ground plane boolean operation DONE\n")
        
        
        # bar.text("Silicon Nitride Deposition")
        # this creates the dielectric layer by doing a boolean operation on all the shapes added to the positive dielectric call and the shapes added to the dielectric cutouts cell
        dielectric_layer = gdspy.boolean(self.silicon_nitride_positives.get_polygons([3,0]), self.silicon_nitride_cutouts, "not", precision=0.01, **self.SiN_dep) # note the positives need to be a polygon set so you get polygons with tupple (3,0) which is layer 3 datatype 0 i.e. the SiN layer
        ...
        # bar.text("Silicon Oxide")
        SiO_layer = gdspy.boolean(self.silicon_oxide_positives.get_polygons([5,0]), self.silicon_oxide_cutouts, "not", precision=0.01, **self.SiO) # note the positives need to be a polygon set so you get polygons with tupple (3,0) which is layer 3 datatype 0 i.e. the SiN layer
        ...
        # bar.text("Silicon Nitride Membrane")
        SiN_membrane_layer = gdspy.boolean(self.silicon_nitride_membrane_positives.get_polygons([6,0]), self.silicon_nitride_membrane_cutouts, "not", precision=0.01, **self.SiN_Membrane) # note the positives need to be a polygon set so you get polygons with tupple (3,0) which is layer 3 datatype 0 i.e. the SiN layer
